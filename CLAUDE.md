...
| `build.loft` | ⚠⚠ **THE BUILD ORDER — the only way a structure comes into existence during a run** (`plans/27`).  A site, what goes there, and the work still owed, in a sparse layer on `WaveState`.  ⚠ Work is stored as **SPENT, never LEFT**, in INTEGER base units: [loft#914] makes a partial literal read as *nobody has started* rather than *already finished*, and a float rate accumulated per tick comes up a tick short (`@M049`, `@D003`'s family).  ⚠⚠ **The total is a DURATION derived from the kind** — 10 s a wall, 20 s a `wall_high`, 30 s a tower — and it is FLAT rather than `structure_max_hp`, because a wall is cheaper to BREAK at an end and not cheaper to BUILD there (`@M048`).  ⚠⚠ **`fixstep::Timer` was refused on semantics**: it fires once and DISARMS, and a progress bar must stay readable after it completes.  ⚠ `order_place` asks `order_fault` itself, so an illegal order cannot be placed by forgetting to check; `buildable` is the palette flag it finally reads, unused since plan 01.  ⚠⚠ **One queue, two things to build** (`@X273`): `what` is `BUILD_GROUND` / `BUILD_TOWER` and is read at exactly ONE site, `build_resolve` — delete that branch and a tower silently paints SEA.  ⚠⚠ **A TOWER ORDER REFUSES ERASURE** (`@X274`) — the points went at pickup, and the wall trail erases anything it drives over |
| `persist.loft` | ⚠⚠ **A PLANET — a place that REMEMBERS** (BACKLOG B3).  `dryopea_planets/<planet>/<player>/world.json`, opened with `make play PLANET=<name>`.  ⚠ It is `@X182`'s *sole prerequisite of five multiplayer features* — none of which needs mmap; they need a place and a KEY.  ⚠⚠ **The paths are DERIVED from the key, never passed**, or a caller could point a session at another player's world.  ⚠ The PLAYER is in the path from day one though dryopea has one (`@X188`).  ⚠⚠ **A planet is the GROUND and the MARKERS and NOT a run** — a wall the crew raised survives because it is ground; the order, the roster and the wallet do not.  ⚠⚠ **JSON, not the mmap `Store`, on MEASURED grounds** (`@M052`): `store_persist_bind` shipped and works cross-process, but dryopea's world is a FIELD of `EditorState`, so a bind writes the EDITOR's store — undo history and all — and any new editor field silently invalidates every saved world |
| `errand.loft` | ⚠⚠ **A ROBOT GOING ABOUT ITS BUSINESS** (BACKLOG B4) — `Traffic` (a rate + a `Bank`), `traffic_due`, `errand_done`, `errand_depart`.  ⚠ **No mover and no second AI**: `enemy_walk_heading` has walked a robot along its heading since plan 11 F5b, and `SETTING.md` says a spawn marker's direction IS *what they were going before*.  What was missing was at both ends — a business that never ENDED (walk into a cliff and it stood there for the run) and nothing that ever STARTED one the schedule had not.  ⚠⚠ **The bubble takes the errand, ONE WAY** — `spawn.loft::wave_cutoff` clears it, sweeping the roster before anybody moves so it reads the position a mover would test for itself, and the two cannot disagree about which robots are lost.  ⚠ It was one line of `enemy_step` until BACKLOG C3, which needed the JAMMER to gate it.  ⚠⚠ **The same traffic is scenery or a wave and the ROUTE decides, not the distance** (`@M053`): a road that runs PAST leaves the wallet at 200 of 200, one that runs THROUGH drains it.  ⚠ **Departing is not DYING** — no body, no salvage, no payment.  ⚠ `Enemy.errand` and the rate both default to *off*, which is why 679 measurements did not move |
| `skill.loft` | ⚠⚠ **CREW SKILLS — build, repair, scout** (BACKLOG C1).  `Skills` on `Helper` (identity is the roster slot), `skill_factor`, `skill_work_units`, and the DETECTION rule.  ⚠ **The crew have skills and the PLAYER does not** (`@X119`): the pilot's repair stays a flat 30 ticks.  ⚠⚠ **Level 0 is bit-for-bit the old game** — `skill_factor` answers exactly 1.0, and the guard lives THERE because a second one in `skill_work_units` could be deleted with the gate still green; its real job is the NEGATIVE level (−0.5 without it).  ⚠ The curve is CHOSEN and says so — the archive names the twelve skills and gives no numbers — so the gate asserts PROPERTIES, not constants.  ⚠⚠ **DETECTION IS TWO RADII THAT INTERACT** (`@X277`): the SUBJECT owns half, so a trained scout notices *quieter* things rather than seeing further, and **detection is not REACH** |
| `endure.loft` | ⚠⚠ **ENDURANCE — work spends it, rest restores it** (BACKLOG C2, `@X113`).  The crew-side of the tower's charge.  ⚠ **A tired person works LESS and never stops** — `PROGRESSION.md` refuses the cliff, so it bottoms out at a FLOOR.  ⚠⚠ **Rest is CONTINUOUS** (every alive crew member, every tick) and working outpaces it, which is what lets the three jobs spend independently with nothing to co-ordinate — the alternative needs a per-tick scratch flag on a person.  ⚠⚠ **Skill makes you PRODUCTIVE, not TIRELESS**: a site tires by the RAW elapsed, never the scaled output.  ⚠ **You tire from work you actually DID** — spending inside the composed door charged every helper for repairing nothing, 1.67x a tick per tick.  ⚠ Sized so the corpus cannot reach it (`@M055`): ~675 ticks to tire against a longest base of 321, and the arithmetic is ASSERTED so a longer base goes red first |
| `jammer.loft` | ⚠⚠ **THE JAMMER SWITCH — turning your own core off** (BACKLOG C3, `@X280`).  `Jammer` + `jammer_on` / `jammer_set` / `jammer_toggle` / `jammer_loot_rate`, plus `spawn.loft`'s `wave_cutoff` and `wave_jammer_switch`.  ⚠ The core IS the jammer, so it does three things the fiction makes one: it CUTS a robot off, which is why there are waves, which is why there is anything to salvage.  ⚠⚠ **It stops the SUPPLY and never the SIEGE** — losing an errand is one way (`@X276`), so a player who hits the switch with a wave on the perimeter has bought nothing for that wave, and the SCRAMBLE is still the answer to being overrun.  ⚠⚠ **The salvage stops as INCOME, never as WORK**: the heap still clears, you are just working for free — which is also why toggling cannot be farmed, both effects being CONTINUOUS.  ⚠⚠ **The field stores `off`** so [loft#914]'s silent default is today's game; every reader asks `jammer_on` and never the field.  ⚠ `jammer_loot_rate` is the ONE door the income goes through, because `spawn.loft` pays the player and the crew at two sites.  ⚠ It decides no END GAME — `DESIGN.md` § Shutting down the scrambler needs a tier 3 that does not exist |
| `font.loft` | ⚠⚠ **THE FONT — the ONE seam to `graphics::draw_text`** (BACKLOG B1).  dryopea rasterises NOTHING; this owns *which* font, *where* it lives and what one that did not load may do.  ⚠⚠ **The path is ABSOLUTE and that is a requirement, ENFORCED at the door**: `gl_load_font` reads a relative path against the process CWD in the pinned `graphics` 0.5.2 and against `source_dir()` in 0.8.0, so `loft install graphics` would silently stop the font loading (`@X268`) — and `font_load_from` REFUSES a relative path, because `is_file` says such a path is fine and cannot see the difference.  ⚠⚠ **A font that failed to load draws in whatever font loaded FIRST** — a null handle collapses to 0, which is live — so the failure is the wrong typeface, not a blank corner (`@M047`), and every door asks `loaded`.  ⚠ `loaded` is a second field for one fact ON PURPOSE: under [loft#914] a partial literal FAILS CLOSED that way and would fail INTO the trap the other.  ⚠⚠ It decides no COMPOSITING — 774 of a glyph line's 1324 pixels are BLENDED, so text in a classified frame breaks `@X077` and `@X092`; that is the first consumer's call |
| `picker.loft` / `hud.loft` / `editor_mode.loft` / `chunks.loft` / `history.loft` | palette UI, HUD, the mode flag, the dirty-chunk set, undo/redo.  ⚠ Since plan 19 P7 `hud.loft` also owns **the ONE number the game shows** — `digit_segments` / `render_digit` / `render_number` / `hud_wallet_points`.  ⚠⚠ **The digits are RECTANGLES, and since BACKLOG B1 that is a CHOICE rather than forced** (`@X097`, `@X268`): text now draws headless, so what keeps them is that anti-aliasing puts 58% of a glyph in `unknown` (`@M047`), plus the independent oracle and the 24x40 legibility finding (`@M042`).  ⚠ A wrong row in the segment table draws a good-looking digit that reads the wrong number, so it is gated by an INDEPENDENT ORACLE — the lit-segment counts 6 2 5 5 4 5 6 3 7 6.  ⚠⚠ Since `@X098` it also owns the wallet's amber-to-red RAMP — a function of the INTEGER the digits show, which is what makes its reachable colours finite (201) and lets a test sweep them all |
...
| [docs/PLAYING.md](docs/PLAYING.md) | ⚠ **How to play what exists** (BACKLOG A4) — every key, what happens without pressing one, and what is not there yet.  ⚠⚠ **The one controls list, and it is GATED** by `tests/a4_the_controls.loft` against `bindings.loft::editor_actions`: every action needs a table row and every palette key must be named on the same row as the ground it paints.  It exists because `main.loft`'s hand-kept copy was **off by one** for twenty-five plans (`@X267`) |
...
| [docs/TOOLCHAIN.md](docs/TOOLCHAIN.md) | ⚠⚠ **How the gates go red for reasons that are not defects** — the 300 s hard-kill, two suites clobbering each other, a wall clock that is not yours alone, and the `graphics` cdylib fault (with two tidy explanations already FALSIFIED, so nobody re-derives them).  ⚠ Two of these read exactly like each other.  ⚠⚠ And § The BINARY moves under you now covers the case where **nothing goes red at all**: the SUITE TOTAL moved 1407 → 1395 with no test file changed, because `loft test` collects zero-argument helpers and the rule moved under it (`@M044`) |
...
| [assets/README.md](assets/README.md) | ⚠ **The binary content the running game loads** — one file today, `DejaVuSans-Bold.ttf`.  ⚠⚠ **Its licence travels with it because that licence says it must**; Liberation Sans Narrow was refused on LICENCE (GPL-2-with-exception) rather than looks.  Changing the font is one constant, `src/font.loft::TEXT_FONT_FILE` |
