...
| Pick something CONCRETE to build | [`plans/BACKLOG.md`](plans/BACKLOG.md) — ⚠⚠ **deliberately unordered**, grouped by what each item unblocks.  ⚠ Two facts worth knowing and neither forces a sequence: **A1 is small and is what lets the game be played at all today**, and **B1 (text) has the widest blast radius of anything on the list** |
...
| Play the game in a window | ⚠⚠ **`make play` opens an EMPTY world** — `maps/` is empty and `.keys` cannot be loaded into a live session, so **there is no base to play** ([`plans/BACKLOG.md`](plans/BACKLOG.md) A1, the biggest blocker on playing today).  ⚠ What works: paint ground and place markers by hand, then press **P** (plan 19 P3).  The crew lands at the core, WASD drives it, and waves arrive on the wall clock.  ⚠ **Nothing of the game is drawn yet** — P4 — so the console echo (a line per tick) is the whole of what you can see, and the camera does not follow the vehicle |
