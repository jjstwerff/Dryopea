...
| [docs/EXAMPLES.md](docs/EXAMPLES.md) | ⚠ **The worked-example convention** — a public function is documented by the TESTS that show how to use it, cited by a stable marker (`// Example: fs001`) that `scripts/examples.sh` resolves.  ⚠ **NEW work only** (project owner, 2026-08-17): there is no retroactive sweep of the 387 existing public functions, and a file opts in with `// #examples` when somebody is already working in it.  ⚠ Dangling / duplicate / orphan citations all FAIL, and the gate carries a five-fixture `--self-test` because with no citations in the repo it is otherwise green over an empty set — `plans/21` § R1's trap |
...
| Document a new public function, or cite a test as its EXAMPLE | [`docs/EXAMPLES.md`](docs/EXAMPLES.md) — put a marker in the test's name (`fn test_fs001_…`) and cite it from the function (`// Example: fs001`).  ⚠ **Only a CITED test carries a marker**: a marker is a contract, so renaming a marked test breaks `scripts/test.sh`, while an unmarked test stays free to refactor.  ⚠ A snippet in prose ROTS (nothing compiles it) and an unlinked test is INVISIBLE — the pair is the deliverable.  ⚠ Write a NEW test if no existing one is clear: a test that proves a function works is not automatically one that shows how to use it |
