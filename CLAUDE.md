...
| gate | command | today |
|---|---|---|
| tests | `scripts/test.sh` | **1531 green**, ~320 s on a busy box, 115 files |
| scenarios | `scripts/validate.sh` | **36 scripts, 687 measurements**, ~14 s |
...
| `errand.loft` | ⚠⚠ **A ROBOT GOING ABOUT ITS BUSINESS** (BACKLOG B4) — `Traffic` (a rate + a `Bank`), `traffic_due`, `errand_done`, `errand_depart`.  ⚠ **No mover and no second AI**: `enemy_walk_heading` has walked a robot along its heading since plan 11 F5b, and `SETTING.md` says a spawn marker's direction IS *what they were going before*.  What was missing was at both ends — a business that never ENDED (walk into a cliff and it stood there for the run) and nothing that ever STARTED one the schedule had not.  ⚠⚠ **The bubble takes the errand, ONE WAY** — `spawn.loft::enemy_step` clears it at the same test the steering mode turns on, so the two cannot disagree about which robots are lost.  ⚠⚠ **The same traffic is scenery or a wave and the ROUTE decides, not the distance** (`@M053`): a road that runs PAST leaves the wallet at 200 of 200, one that runs THROUGH drains it.  ⚠ **Departing is not DYING** — no body, no salvage, no payment.  ⚠ `Enemy.errand` and the rate both default to *off*, which is why 679 measurements did not move |
