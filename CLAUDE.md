...
| Ask what a new player's FIRST FIFTEEN MINUTES are | ⚠⚠ **It describes the FINISHED game and is a TEST, not a task** (`@X251`) — read it as *does this feature serve that opening?*, never as a work queue; the critical path is still BUILDING.  ⚠ **Scouting, not building** (`@X245`, [`docs/EXPLORATION.md`](docs/EXPLORATION.md) § X0b) — a world with trees, insects on them, maybe an elemental moving, and **robots in their normal work flow**.  ⚠⚠ **It needs three things and only one exists**: a world to look at (⚠⚠ **THE GAP — every robot in the code is a WAVE; there is no mover for a robot going about its business**), somebody to point at it (blocked on TEXT, which this promotes to **the gate on the opening**), and somewhere to go (§ X1's rings).  ⚠⚠ **The landing zone is NEUTRAL** (`@X246`) — small operations nearby, the big ones elsewhere — which is a difficulty gradient built into the geography and what makes the unbounded recon window safe.  ⚠⚠ **Walls are UNCONDITIONAL** (`@X247`): the crew can always dig, so a bad landing choice leaves you worse off and never defenceless |
