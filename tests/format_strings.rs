...
//
// Note: nested *string* literals inside `{...}` crash the compiler with "Dual
// definition of ..." (PROBLEMS #9).  Those cases are `#[ignore]`-d and documented.
...
// ---------------------------------------------------------------------------
// String literal nested inside `{...}` — known crash (PROBLEMS #9).
// The lexer does not support a second level of `"..."` inside a format
// expression; it triggers a "Dual definition of" assertion failure.
// These tests are `#[ignore]`-d until the lexer is fixed.
// ---------------------------------------------------------------------------
