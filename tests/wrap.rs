...
macro_rules! script_test {
    ($name:ident, $path:literal) => {
        ...
        fn $name() -> std::io::Result<()> {
            let _g = WRAP_LOCK.lock().unwrap_or_else(|e| e.into_inner());
            run_test(PathBuf::from($path), false)
        }
    };
}
...
/// Ignored by default while issue 27 (set_int crash in 16-parser.loft) is open.
/// Re-enable by running: cargo test -- last --ignored
#[test]
#[ignore]
fn last() -> std::io::Result<()> {
    let _g = WRAP_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    run_test(PathBuf::from("tests/docs/16-parser.loft"), false)
}
...
fn threading() -> std::io::Result<()> {
    let _g = WRAP_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    run_test(PathBuf::from("tests/docs/19-threading.loft"), false)
}
...
fn logging() -> std::io::Result<()> {
    let _g = WRAP_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    run_test(PathBuf::from("tests/docs/20-logging.loft"), false)
}
...
fn file_debug() -> std::io::Result<()> {
    let _g = WRAP_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    run_test(PathBuf::from("tests/docs/13-file.loft"), true)
}

/// Debug the run of `16-parser.loft` with a full execution trace written to
/// `tests/dumps/16-parser.loft.txt`.  Use this to diagnose the store_nr=60 crash.
/// Ignored by default — the execution trace takes ~40 s and the file is known broken.
/// Run explicitly: cargo test -- parser_debug --ignored
#[test]
#[ignore]
fn parser_debug() -> std::io::Result<()> {
    let _g = WRAP_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    run_test(PathBuf::from("tests/docs/16-parser.loft"), true)
}
...
fn run_test(entry: PathBuf, debug: bool) -> std::io::Result<()> {
    ...
    if debug {
        ...
        #[cfg(debug_assertions)]
        if std::env::var("LOFT_DUMP").is_ok() {
            let config = LogConfig::from_env();
