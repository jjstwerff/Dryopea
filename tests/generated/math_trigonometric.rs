...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: f64 = { //block_2: float
    let _pre1 = t_5float_sin(stores, std::f64::consts::PI);
    let _pre2 = (t_5float_cos(stores, std::f64::consts::PI)) * (external::op_conv_float_from_int((1000_i32)));
    let _pre0 = (_pre1) + (_pre2);
    t_5float_ceil(stores, _pre0)
  } /*block_2: float*/;
  ;
  if ((var_test_value) - (external::op_conv_float_from_int((external::op_min_single_int((1000_i32)))))).abs() < 0.000_000_001f64 {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_float(&mut (var___work_1), (var_test_value), (0_i32), (0_i32));
      (var___work_1).push_str(&(" != -1000".to_string()));
      var___work_1
