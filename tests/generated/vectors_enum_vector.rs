...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    ...
    {let db = (var__elm_1); let _val = i32::from((2_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    stores.record_finish(&(var_v), &(var__elm_1), (20_i32) as u16, (65535_i32) as u16);
    if ({let db = (vector::get_vector(&(var_v), (1_i32) as u32, (2_i32), &stores.allocations)); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8}) == 255 { i32::MIN } else { i32::from(({let db = (vector::get_vector(&(var_v), (1_i32) as u32, (2_i32), &stores.allocations)); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8})) }
  } /*block_2: integer*/;
  if (var_test_value) == (2_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 2".to_string()));
      var___work_1
