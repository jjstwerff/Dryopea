...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    { //Add text_3: text["__work_1"]
      (var___work_1).clear();;
      if let Some(c) = char::from_u32((external::text_character(&(var_a), (0_i32))) as u32) { (var___work_1).push(c); };
      if let Some(c) = char::from_u32((external::text_character(&(var_a), (1_i32))) as u32) { (var___work_1).push(c); };
      if let Some(c) = char::from_u32((external::text_character(&(var_a), (2_i32))) as u32) { (var___work_1).push(c); };
      if let Some(c) = char::from_u32((external::text_character(&(var_a), (3_i32))) as u32) { (var___work_1).push(c); };
      if let Some(c) = char::from_u32((external::text_character(&(var_a), (4_i32))) as u32) { (var___work_1).push(c); };
      if let Some(c) = char::from_u32((external::text_character(&(var_a), (5_i32))) as u32) { (var___work_1).push(c); };
      if let Some(c) = char::from_u32((external::text_character(&(var_a), (6_i32))) as u32) { (var___work_1).push(c); };
      (var___work_1).push_str(&(".".to_string()));
      if let Some(c) = char::from_u32((external::text_character(&(var_a), (7_i32))) as u32) { (var___work_1).push(c); };
      {};
      var___work_1
    } /*Add text_3: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  ;
  if (var_test_value) == ("♥♥♥😃😃😃😃.".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
