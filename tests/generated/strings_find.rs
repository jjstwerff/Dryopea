...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    let mut var_v: String = "something".to_string();
    let _pre0 = { //Add text_3: text["__work_1"]
      (var___work_1).clear();;
      (var___work_1).push_str(&("t".to_string()));
      (var___work_1).push_str(&("h".to_string()));
      var___work_1
    } /*Add text_3: text["__work_1"]*/;
    let _ret = if let Some(v) = (var_v).find((_pre0).as_str()) { v as i32 } else { i32::MIN };
    {};
    _ret
  } /*block_2: integer*/;
  if (var_test_value) == (4_i32) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      ...
      (var___work_2).push_str(&(" != 4".to_string()));
      var___work_2
