...
fn n_test(stores: &mut Stores) { //block_1: void
      ...
      loop { //For loop_4
        let mut var_e: i32 = { //Vector Index_5: integer
          let _pre1 = { //Iter range_6: integer
            var__index_2 = if !(external::op_conv_bool_from_int((var__index_2))) {3_i32} else {external::op_min_int((var__index_2), (1_i32))};
            if (var__index_2) < (0_i32) {break} else {()};
            var__index_2
  ...
  if (var_test_value) == (8421_i32) {()} else {panic!("{}", ({ //Formatted string_8: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 8421".to_string()));
      var___work_1
