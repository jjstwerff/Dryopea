...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    if (3_i32) < (external::op_add_int((2_i32), (2_i32))) { //block_3: text
      "A".to_string()
    } /*block_3: text*/ else { //block_4: text
      "B".to_string()
    } /*block_4: text*/.to_string()
  } /*block_2: text*/;
  ;
  if (var_test_value) == ("A".to_string()) {()} else {panic!("{}", ({ //Formatted string_5: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_text(&mut (var___work_1), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&(" != \"A\"".to_string()));
      var___work_1
