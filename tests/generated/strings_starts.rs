...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: bool = { //block_2: boolean
    ("something".to_string()).starts_with(("someone".to_string()).as_str())
  } /*block_2: boolean*/;
  if (var_test_value) == (false) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != false".to_string()));
      var___work_1
