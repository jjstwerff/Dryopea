...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    ;
    let mut var_i: DbRef = DbRef {store_nr: (var_v).store_nr, rec: (var_v).rec, pos: (var_v).pos + (0_i32) as u32};
    ;
    if (if ({let db = (var_i); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8}) == 255 { i32::MIN } else { i32::from(({let db = (var_i); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8})) }) == (if (2_u8) == 255 { i32::MIN } else { i32::from((2_u8)) }) {()} else {panic!("{}", ("Compare".to_string()));};
    ;
    let mut var_w: DbRef = todo!();
    ;
    todo!();
    ;
    { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(var_v), (30_i32) as u16));
      (var___work_1).push_str(&(" & ".to_string()));
      (var___work_1).push_str(&stores.rec(&(var_i), (18_i32) as u16));
      drop((var_v));
      drop((var_w));
      var___work_1
  ...
  ;
  if (var_test_value) == ("{key_fields:[],data:Text {t_value:\"Something\"}} & Text {t_value:\"Something\"}".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"{key_fields:[],data:Text {t_value:\"Something\"}} & Text {t_value:\"Something\"}\"".to_string()));
      var___work_2
