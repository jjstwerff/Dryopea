...
fn n_test(stores: &mut Stores) { //block_1: void
    ...
    { //For block_3: void
      ...
      loop { //For loop_4
        let mut var_a: i32 = { //Iter range_5: integer
          var_a__index = if !(external::op_conv_bool_from_int((var_a__index))) {6_i32} else {external::op_min_int((var_a__index), (1_i32))};
          if (var_a__index) < (1_i32) {break} else {()};
          var_a__index
  ...
  if (var_test_value) == (654321_i32) {()} else {panic!("{}", ({ //Formatted string_7: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 654321".to_string()));
      var___work_1
