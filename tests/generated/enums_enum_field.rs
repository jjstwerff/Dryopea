...
fn n_fill(stores: &mut Stores) -> DbRef { //block_1: ref(Container)
  ...
  { //Object_2: ref(Container)["__ref_1"]
    ...
    {let db = (var___ref_1); let s_val = ("testing".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (16_i32) as u32, s_pos as i32);};
    {let db = (var___ref_1); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (DbRef {store_nr: (var___ref_1).store_nr, rec: (var___ref_1).rec, pos: (var___ref_1).pos + (0_i32) as u32}); let _val = (1234.56_f32); stores.store_mut(&db).set_single(db.rec, db.pos + (4_i32) as u32, _val);};
    {let db = (DbRef {store_nr: (var___ref_1).store_nr, rec: (var___ref_1).rec, pos: (var___ref_1).pos + (0_i32) as u32}); let _val = i32::from((3_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    {let db = (var___ref_1); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (20_i32) as u32, _val);};
    let mut var__elm_1: DbRef = stores.record_new(&(var___ref_1), (23_i32) as u16, (2_i32) as u16);
    {let db = (var__elm_1); let _val = (9876543210_i64); stores.store_mut(&db).set_long(db.rec, db.pos + (8_i32) as u32, _val);};
    {let db = (var__elm_1); let _val = i32::from((1_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    stores.record_finish(&(var___ref_1), &(var__elm_1), (23_i32) as u16, (2_i32) as u16);
    var__elm_1 = stores.record_new(&(var___ref_1), (23_i32) as u16, (2_i32) as u16);
    {let db = (var__elm_1); let s_val = ("An example sentence of text".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = i32::from((4_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    stores.record_finish(&(var___ref_1), &(var__elm_1), (23_i32) as u16, (2_i32) as u16);
    var__elm_1 = stores.record_new(&(var___ref_1), (23_i32) as u16, (2_i32) as u16);
    {let db = (var__elm_1); let _val = (3.141592653589793_f64); stores.store_mut(&db).set_float(db.rec, db.pos + (8_i32) as u32, _val);};
