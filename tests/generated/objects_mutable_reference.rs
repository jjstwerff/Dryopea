...
fn init(db: &mut Stores) {
    ...
    db.field(s, "num", 0);
    let vec_values = db.vector(0);
    db.field(s, "values", vec_values);
    db.finish();
}
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    let mut var_v: DbRef = stores.null();
    (var_v) = stores.alloc_record((19_i32) as u16);
    {let db = (var_v); let _val = (1_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var_v); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
    n_add(stores, todo!(), 2_i32);
    n_add(stores, todo!(), 3_i32);
    { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(var_v), (19_i32) as u16));
      drop((var_v));
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  if (var_test_value) == ("{num:6,values:[2,3]}".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"{num:6,values:[2,3]}\"".to_string()));
      var___work_2
