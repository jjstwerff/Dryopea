...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: f64 = { //block_2: float
    t_5float_round(stores, (std::f64::consts::PI) * (1000_f64))
  } /*block_2: float*/;
  if ((var_test_value) - (external::op_conv_float_from_int((3142_i32)))).abs() < 0.000_000_001f64 {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_float(&mut (var___work_1), (var_test_value), (0_i32), (0_i32));
      (var___work_1).push_str(&(" != 3142".to_string()));
      var___work_1
