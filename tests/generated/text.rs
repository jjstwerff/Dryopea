...
fn n_assert(stores: &mut Stores, stack: &mut DbRef) {
    let v_message = *stores.get::<String>(stack);
...
fn t_4File_write(stores: &mut Stores, stack: &mut DbRef) {
    let v_v = *stores.get::<String>(stack);
...
fn n_env_variable(stores: &mut Stores, stack: &mut DbRef) {
    let v_name = *stores.get::<String>(stack);
    let new_value = { Stores::os_variable(&v_name.str()) };
    stores.put(stack, new_value);
}

fn t_4text_starts_with(stores: &mut Stores, stack: &mut DbRef) {
    let v_value = *stores.get::<String>(stack);
    let v_self = *stores.get::<String>(stack);
    let new_value = { v_self.str().starts_with(v_value.str().as_str()) };
    stores.put(stack, new_value);
}
...
fn t_4text_ends_with(stores: &mut Stores, stack: &mut DbRef) {
    let v_value = *stores.get::<String>(stack);
    let v_self = *stores.get::<String>(stack);
    let new_value = { v_self.str().ends_with(v_value.str().as_str()) };
    stores.put(stack, new_value);
}

fn t_4text_trim(stores: &mut Stores, stack: &mut DbRef) {
    let v_both = *stores.get::<String>(stack);
    let new_value = { v_both.str().trim().to_string() };
    stores.put(stack, new_value);
}

fn t_4text_trim_start(stores: &mut Stores, stack: &mut DbRef) {
    let v_self = *stores.get::<String>(stack);
    let new_value = { v_self.str().trim_start().to_string() };
    stores.put(stack, new_value);
}

fn t_4text_trim_end(stores: &mut Stores, stack: &mut DbRef) {
    let v_self = *stores.get::<String>(stack);
    let new_value = { v_self.str().trim_end().to_string() };
    stores.put(stack, new_value);
}
...
fn t_4text_find(stores: &mut Stores, stack: &mut DbRef) {
    let v_value = *stores.get::<String>(stack);
    let v_self = *stores.get::<String>(stack);
    let new_value = { if let Some(v) = v_self.str().find(v_value.str().as_str()) { v as i32 } else { i32::MIN } };
    stores.put(stack, new_value);
}
...
fn t_4text_rfind(stores: &mut Stores, stack: &mut DbRef) {
    let v_value = *stores.get::<String>(stack);
    let v_self = *stores.get::<String>(stack);
    let new_value = { if let Some(v) = v_self.str().rfind(v_value.str().as_str()) { v as i32 } else { i32::MIN } };
    stores.put(stack, new_value);
}
...
fn t_4text_contains(stores: &mut Stores, stack: &mut DbRef) {
    let v_value = *stores.get::<String>(stack);
    let v_self = *stores.get::<String>(stack);
    let new_value = { v_self.str().contains(v_value.str().as_str()) };
    stores.put(stack, new_value);
}

fn t_4text_replace(stores: &mut Stores, stack: &mut DbRef) {
    let v_with = *stores.get::<String>(stack);
    let v_value = *stores.get::<String>(stack);
    let v_self = *stores.get::<String>(stack);
    let new_value = { v_self.str().replace(v_value.str().as_str(), v_with.str().as_str()) };
    stores.put(stack, new_value);
}
