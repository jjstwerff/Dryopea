...
fn n_data(stores: &mut Stores, mut var_n: String, mut var_res: DbRef) -> String { //block_1: text["res"]
  ...
  {let db = (var_res); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (0_i32) as u32) as u32))}.to_string()
} /*block_1: text["res"]*/
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    var_test_value = { //block_2: text["__ref_1"]
    n_data(stores, "test".to_string().to_string(), var___ref_1).to_string()
  } /*block_2: text["__ref_1"]*/;
  if (var_test_value) == ("test".to_string()) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_text(&mut (var___work_1), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&(" != \"test\"".to_string()));
      var___work_1
