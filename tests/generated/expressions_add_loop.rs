  ...
  let mut var_test_value = String::new();
    ...
    { //For block_3: void
      ...
      loop { //For loop_4
        ...
        { //block_6: void
          if let Some(c) = char::from_u32((char::from_u32(48_u32).unwrap_or('\0')) as u32) { (var_b).push(c); };
        } /*block_6: void*/;
      } /*For loop_4*/;
    } /*For block_3: void*/;
    var_b
  } /*block_2: text["b"]*/;
  if (var_test_value) == ("00000".to_string()) {()} else {panic!("{}", ({ //Formatted string_7: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_text(&mut (var___work_1), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&(" != \"00000\"".to_string()));
      var___work_1
