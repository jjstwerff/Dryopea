...
fn init(db: &mut Stores) {
    ...
    db.field(s, "vector", vec_vector);
    db.vector(14);
    let e = db.enumerate("Number");
    db.value(e, "One", u16::MAX);
    db.value(e, "Two", u16::MAX);
    db.value(e, "Three", u16::MAX);
    db.value(e, "Four", u16::MAX);
    db.finish();
}
...
fn n_test(stores: &mut Stores) { //block_1: void
  let mut var___work_1: String = "".to_string();
  ;
  let mut var_test_value: bool = { //block_2: boolean
    (if (stores.to_enum((18_i32) as u16, &("Two".to_string()))) == 255 { i32::MIN } else { i32::from((stores.to_enum((18_i32) as u16, &("Two".to_string())))) }) < (if (stores.to_enum((18_i32) as u16, &("Four".to_string()))) == 255 { i32::MIN } else { i32::from((stores.to_enum((18_i32) as u16, &("Four".to_string())))) })
  } /*block_2: boolean*/;
  ;
  if (var_test_value) == (true) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_text(&mut (var___work_1), &(if (var_test_value) {"true"} else {"false"}), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&(" != true".to_string()));
      var___work_1
