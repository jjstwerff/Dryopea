...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: f64 = { //block_2: float
    let mut var_a: DbRef = todo!();
    let _ret = ({let db = (vector::get_vector(&(var_a), (8_i32) as u32, (2_i32), &stores.allocations)); stores.store(&db).get_float(db.rec, db.pos + (0_i32) as u32)}) + ({let db = (vector::get_vector(&(var_a), (8_i32) as u32, (3_i32), &stores.allocations)); stores.store(&db).get_float(db.rec, db.pos + (0_i32) as u32)});
    drop((var_a));
    _ret
  } /*block_2: float*/;
  ;
  if ((var_test_value) - (18120.00000001001_f64)).abs() < 0.000_000_001f64 {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_float(&mut (var___work_1), (var_test_value), (0_i32), (0_i32));
      (var___work_1).push_str(&(" != 18120.00000001001".to_string()));
      var___work_1
