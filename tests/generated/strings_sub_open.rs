...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    external::sub_text(&(var_a), (2_i32), (2147483647_i32)).to_string().to_string()
  } /*block_2: text["a"]*/;
  if (var_test_value) == ("345".to_string()) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_text(&mut (var___work_1), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&(" != \"345\"".to_string()));
      var___work_1
