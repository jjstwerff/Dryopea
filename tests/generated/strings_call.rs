...
fn n_choice(stores: &mut Stores, mut var_a: String, mut var_b: String) -> String { //block_1: text["a", "b"]
  ;
  if (t_4text_len(stores, var_b.clone())) < (t_4text_len(stores, var_a.clone())) { //block_2: text["a"]
    var_a
  } /*block_2: text["a"]*/ else { //block_3: text["a"]
    var_b
  } /*block_3: text["a"]*/.to_string()
} /*block_1: text["a", "b"]*/
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    var_test_value = { //block_2: text["__work_3"]
    { //Add text_3: text["__work_3"]
      ...
      let _pre1 = { //Formatted string_5: text["__work_2"]
        var___work_2 = "".to_string();
        external::format_long(&mut (var___work_2), (external::op_conv_long_from_int((2_i32))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
        (var___work_2).push_str(&("1".to_string()));
        var___work_2
      } /*Formatted string_5: text["__work_2"]*/;
      (var___work_3).push_str(&(n_choice(stores, _pre0.to_string(), _pre1.to_string())));
      (var___work_3).push_str(&(n_choice(stores, "2".to_string().to_string(), "".to_string().to_string())));
      var___work_3
    } /*Add text_3: text["__work_3"]*/.to_string()
  } /*block_2: text["__work_3"]*/;
  ;
  if (var_test_value) == ("0012".to_string()) {()} else {panic!("{}", ({ //Formatted string_6: text["__work_4"]
      var___work_4 = "Test failed ".to_string();
      external::format_text(&mut (var___work_4), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_4).push_str(&(" != \"0012\"".to_string()));
      var___work_4
