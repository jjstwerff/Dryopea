...
fn t_7Integer_add(stores: &mut Stores, mut var_self: DbRef) -> i32 { //block_1: integer
  {let db = (var_self); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }
} /*block_1: integer*/

fn t_4Text_add(stores: &mut Stores, mut var_self: DbRef) -> i32 { //block_1: integer
  ({let db = (var_self); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (4_i32) as u32) as u32))}).parse().unwrap_or(i32::MIN)
} /*block_1: integer*/

fn t_5Array_add(stores: &mut Stores, mut var_self: DbRef) -> i32 { //block_1: integer
  let mut var_n: i32 = 0_i32;
  { //For block_2: void
    let mut var__vector_1: DbRef = DbRef {store_nr: (var_self).store_nr, rec: (var_self).rec, pos: (var_self).pos + (4_i32) as u32};
    let mut var_v__index: i32 = -1_i32;
    loop { //For loop_3
      let mut var_v: i32 = { //iter next_4: integer
        var_v__index = external::op_add_int((var_v__index), (1_i32));
        {let db = (vector::get_vector(&(var__vector_1), (4_i32) as u32, (var_v__index), &stores.allocations)); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (0_i32) as u32)} }
      } /*iter next_4: integer*/;
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    (var___ref_1) = stores.alloc_record((23_i32) as u16);
    let mut var_l: DbRef = DbRef {store_nr: (var___ref_1).store_nr, rec: (var___ref_1).rec, pos: (var___ref_1).pos + (0_i32) as u32};
    {let db = (var___ref_1); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    let mut var__elm_1: DbRef = stores.record_new(&(var_l), (24_i32) as u16, (65535_i32) as u16);
    {let db = (var__elm_1); let s_val = ("123".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = i32::from((2_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    stores.record_finish(&(var_l), &(var__elm_1), (24_i32) as u16, (65535_i32) as u16);
    var__elm_1 = stores.record_new(&(var_l), (24_i32) as u16, (65535_i32) as u16);
    {let db = (var__elm_1); let _val = (101_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
    {let db = (var__elm_1); let _val = i32::from((1_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    stores.record_finish(&(var_l), &(var__elm_1), (24_i32) as u16, (65535_i32) as u16);
    var__elm_1 = stores.record_new(&(var_l), (24_i32) as u16, (65535_i32) as u16);
    {let db = (var__elm_1); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
    let mut var__elm_2: DbRef = stores.record_new(&(var__elm_1), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_2); let _val = (1_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var__elm_1), &(var__elm_2), (22_i32) as u16, (1_i32) as u16);
    var__elm_2 = stores.record_new(&(var__elm_1), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_2); let _val = (2_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var__elm_1), &(var__elm_2), (22_i32) as u16, (1_i32) as u16);
    var__elm_2 = stores.record_new(&(var__elm_1), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_2); let _val = (3_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var__elm_1), &(var__elm_2), (22_i32) as u16, (1_i32) as u16);
    var__elm_2 = stores.record_new(&(var__elm_1), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_2); let _val = (4_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var__elm_1), &(var__elm_2), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_1); let _val = i32::from((3_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    stores.record_finish(&(var_l), &(var__elm_1), (24_i32) as u16, (65535_i32) as u16);
    ...
    { //For block_3: void
      ...
      loop { //For loop_4
        let mut var_v: DbRef = { //iter next_5: ref(Value)
          var_v__index = external::op_add_int((var_v__index), (1_i32));
          DbRef {store_nr: (vector::get_vector(&(var__vector_3), (8_i32) as u32, (var_v__index), &stores.allocations)).store_nr, rec: (vector::get_vector(&(var__vector_3), (8_i32) as u32, (var_v__index), &stores.allocations)).rec, pos: (vector::get_vector(&(var__vector_3), (8_i32) as u32, (var_v__index), &stores.allocations)).pos + (0_i32) as u32}
    ...
    let mut var_t: DbRef = if (if ({let db = (DbRef {store_nr: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).store_nr, rec: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).rec, pos: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).pos + (0_i32) as u32}); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8}) == 255 { i32::MIN } else { i32::from(({let db = (DbRef {store_nr: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).store_nr, rec: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).rec, pos: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).pos + (0_i32) as u32}); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8})) }) == (1_i32) {DbRef {store_nr: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).store_nr, rec: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).rec, pos: (vector::get_vector(&(var_l), (8_i32) as u32, (1_i32), &stores.allocations)).pos + (0_i32) as u32}} else {stores.null()};
    { //Formatted string_9: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(var_l), (24_i32) as u16));
      (var___work_1).push_str(&(":".to_string()));
      external::format_long(&mut (var___work_1), (external::op_conv_long_from_int((var_c))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
      (var___work_1).push_str(&(" ".to_string()));
      external::format_long(&mut (var___work_1), (external::op_conv_long_from_int(({let db = (var_t); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
      var___work_1
    } /*Formatted string_9: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  if (var_test_value) == ("[Text {v:\"123\"},Integer {v:101},Array {v:[1,2,3,4]}]:234 101".to_string()) {()} else {panic!("{}", ({ //Formatted string_10: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"[Text {v:\"123\"},Integer {v:101},Array {v:[1,2,3,4]}]:234 101\"".to_string()));
      var___work_2
...
fn t_5Value_add(stores: &mut Stores, mut var_self: DbRef) -> i32 { //dynamic_fn_1: integer
  if (if ({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8}) == 255 { i32::MIN } else { i32::from(({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8})) }) == (1_i32) { //ret_2: void
    return t_7Integer_add(stores, var_self);
  } /*ret_2: void*/ else {()};
  if (if ({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8}) == 255 { i32::MIN } else { i32::from(({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8})) }) == (2_i32) { //ret_3: void
    return t_4Text_add(stores, var_self);
  } /*ret_3: void*/ else {()};
  if (if ({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8}) == 255 { i32::MIN } else { i32::from(({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, 0) as u8})) }) == (3_i32) { //ret_4: void
