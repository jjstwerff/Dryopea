...
fn n_fill(stores: &mut Stores, mut var_c: DbRef) { //block_1: void
  let mut var__elm_1: DbRef = stores.record_new(&(var_c), (20_i32) as u16, (0_i32) as u16);
  {let db = (var__elm_1); let _val = i32::from((1_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (8_i32) as u32, 0, _val);};
  ...
  stores.record_finish(&(var_c), &(var__elm_1), (20_i32) as u16, (0_i32) as u16);
  var__elm_1 = stores.record_new(&(var_c), (20_i32) as u16, (0_i32) as u16);
  {let db = (var__elm_1); let _val = i32::from((2_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (8_i32) as u32, 0, _val);};
  ...
  stores.record_finish(&(var_c), &(var__elm_1), (20_i32) as u16, (0_i32) as u16);
  var__elm_1 = stores.record_new(&(var_c), (20_i32) as u16, (0_i32) as u16);
  {let db = (var__elm_1); let _val = i32::from((3_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (8_i32) as u32, 0, _val);};
  {let db = (var__elm_1); let s_val = ("Two".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
  {let db = (var__elm_1); let _val = (20_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
  stores.record_finish(&(var_c), &(var__elm_1), (20_i32) as u16, (0_i32) as u16);
  var__elm_1 = stores.record_new(&(var_c), (20_i32) as u16, (0_i32) as u16);
  {let db = (var__elm_1); let _val = i32::from((1_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (8_i32) as u32, 0, _val);};
  {let db = (var__elm_1); let s_val = ("Three".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
  {let db = (var__elm_1); let _val = (3_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
  stores.record_finish(&(var_c), &(var__elm_1), (20_i32) as u16, (0_i32) as u16);
  var__elm_1 = stores.record_new(&(var_c), (20_i32) as u16, (0_i32) as u16);
  {let db = (var__elm_1); let _val = i32::from((3_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (8_i32) as u32, 0, _val);};
  {let db = (var__elm_1); let s_val = ("Four".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
  {let db = (var__elm_1); let _val = (4_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
  stores.record_finish(&(var_c), &(var__elm_1), (20_i32) as u16, (0_i32) as u16);
} /*block_1: void*/
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    ...
    (var_c) = stores.alloc_record((20_i32) as u16);
    ...
    n_fill(stores, var_c);
    let _ret = external::op_add_int((external::op_add_int(({let db = (stores.find(&(DbRef {store_nr: (var_c).store_nr, rec: (var_c).rec, pos: (var_c).pos + (4_i32) as u32}), (22_i32) as u16, &[Content::Long((1_u8) as i64), Content::Str(Str::new(("Three".to_string()).as_str()))])); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }), ({let db = (stores.find(&(DbRef {store_nr: (var_c).store_nr, rec: (var_c).rec, pos: (var_c).pos + (4_i32) as u32}), (22_i32) as u16, &[Content::Long((3_u8) as i64), Content::Str(Str::new(("Two".to_string()).as_str()))])); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }))), ({let db = (stores.find(&(DbRef {store_nr: (var_c).store_nr, rec: (var_c).rec, pos: (var_c).pos + (0_i32) as u32}), (21_i32) as u16, &[Content::Str(Str::new(("Four".to_string()).as_str())), Content::Long((4_i32) as i64)])); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }));
    drop((var_c));
    _ret
  } /*block_2: integer*/;
  if (var_test_value) == (27_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 27".to_string()));
      var___work_1
