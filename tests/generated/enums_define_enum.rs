...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  ;
  if (var_test_value) == ("Call {function:\"foo\",parameters:2}".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"Call {function:\"foo\",parameters:2}\"".to_string()));
      var___work_2
