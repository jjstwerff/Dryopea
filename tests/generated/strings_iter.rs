...
fn init(db: &mut Stores) {
    ...
    db.vector(6);
    db.vector(0);
    let s = db.structure("main_vector<character>", 0); // 20
    let vec_vector = db.vector(6);
    db.field(s, "vector", vec_vector);
    let s = db.structure("main_vector<integer>", 0); // 21
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    (var___ref_1) = stores.alloc_record((20_i32) as u16);
    let mut var_a: DbRef = DbRef {store_nr: (var___ref_1).store_nr, rec: (var___ref_1).rec, pos: (var___ref_1).pos + (0_i32) as u32};
    {let db = (var___ref_1); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    (var___ref_2) = stores.alloc_record((21_i32) as u16);
    let mut var_b: DbRef = DbRef {store_nr: (var___ref_2).store_nr, rec: (var___ref_2).rec, pos: (var___ref_2).pos + (0_i32) as u32};
    {let db = (var___ref_2); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    { //For block_3: void
      let mut var_c__index: i32 = 0_i32;
      loop { //For loop_4
        let mut var_c: char = { //for text next_5: character
          let mut var__for_result_1: char = external::text_character(&("123😊🙃😋8".to_string()), (var_c__index));
          var_c__index = external::op_add_int((var_c__index), (((var__for_result_1).len_utf8() as i32)));
          var__for_result_1
        } /*for text next_5: character*/;
        if !(external::op_conv_bool_from_character((var_c))) { //break_6: void
          break;
        } /*break_6: void*/ else {()};
        { //block_7: void
          let mut var__elm_2: DbRef = stores.record_new(&(var_a), (18_i32) as u16, (65535_i32) as u16);
          {let db = (var__elm_2); let _val = (var_c as u32 as i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
          stores.record_finish(&(var_a), &(var__elm_2), (18_i32) as u16, (65535_i32) as u16);
          let mut var__elm_3: DbRef = stores.record_new(&(var_b), (19_i32) as u16, (65535_i32) as u16);
          {let db = (var__elm_3); let _val = (var_c__index); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
          stores.record_finish(&(var_b), &(var__elm_3), (19_i32) as u16, (65535_i32) as u16);
        } /*block_7: void*/;
      } /*For loop_4*/;
    } /*For block_3: void*/;
    { //Formatted string_8: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(var_a), (18_i32) as u16));
      (var___work_1).push_str(&(" indexes:".to_string()));
      (var___work_1).push_str(&stores.rec(&(var_b), (19_i32) as u16));
      var___work_1
    } /*Formatted string_8: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  if (var_test_value) == ("['1','2','3','😊','🙃','😋','8'] indexes:[1,2,3,7,11,15,16]".to_string()) {()} else {panic!("{}", ({ //Formatted string_9: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"['1','2','3','😊','🙃','😋','8'] indexes:[1,2,3,7,11,15,16]\"".to_string()));
      var___work_2
