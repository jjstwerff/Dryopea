...
fn n_obj(stores: &mut Stores) -> DbRef { //block_1: ref(Object)
  ...
  { //Object_2: ref(Object)["__ref_1"]
    (var___ref_1) = stores.alloc_record((18_i32) as u16);
    {let db = (var___ref_1); let _val = (12_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var___ref_1); let s_val = ("hi".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var___ref_1); let _val = (if false {1_i32} else {0_i32}); stores.store_mut(&db).set_byte(db.rec, db.pos + (8_i32) as u32, i32::from((0_i32)), _val);};
    var___ref_1
  } /*Object_2: ref(Object)["__ref_1"]*/
} /*block_1: ref(Object)*/
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    let mut var__field_1: String = {let db = (var_o); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (4_i32) as u32) as u32))}.to_string();
    if let Some(c) = char::from_u32((char::from_u32(33_u32).unwrap_or('\0')) as u32) { (var__field_1).push(c); };
    {let db = (var_o); let s_val = (var__field_1).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(var_o), (18_i32) as u16));
      (var___work_1).push_str(&(" pretty ".to_string()));
      (var___work_1).push_str(&stores.rec(&(var_o), (18_i32) as u16));
      drop((var_o));
      {};
      var___work_1
  ...
  ;
  if (var_test_value) == ("{a:12,bb:\"hi!\",ccc:false} pretty { a: 12, bb: \"hi!\", ccc: false }".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"{a:12,bb:\"hi!\",ccc:false} pretty { a: 12, bb: \"hi!\", ccc: false }\"".to_string()));
      var___work_2
