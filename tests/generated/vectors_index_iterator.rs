...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    ...
    (var_db) = stores.alloc_record((19_i32) as u16);
    {let db = (var_db); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    let mut var__elm_1: DbRef = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    {let db = (var__elm_1); let _val = (101_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var__elm_1); let s_val = ("One".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = (1_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (8_i32) as u32, _val);};
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    var__elm_1 = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    {let db = (var__elm_1); let _val = (92_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var__elm_1); let s_val = ("Two".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = (2_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (8_i32) as u32, _val);};
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    var__elm_1 = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    {let db = (var__elm_1); let _val = (83_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var__elm_1); let s_val = ("Three".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = (3_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (8_i32) as u32, _val);};
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    var__elm_1 = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    {let db = (var__elm_1); let _val = (83_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var__elm_1); let s_val = ("Four".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = (4_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (8_i32) as u32, _val);};
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    var__elm_1 = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    {let db = (var__elm_1); let _val = (83_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var__elm_1); let s_val = ("Five".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = (5_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (8_i32) as u32, _val);};
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    var__elm_1 = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    {let db = (var__elm_1); let _val = (63_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var__elm_1); let s_val = ("Six".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = (6_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (8_i32) as u32, _val);};
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    ;
    if ({let db = (stores.find(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, &[Content::Long((101_i32) as i64), Content::Str(Str::new(("One".to_string()).as_str()))])); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (8_i32) as u32)} }) == (1_i32) {()} else {panic!("{}", ("Missing element".to_string()));};
    ;
    let mut var_sum: i32 = 0_i32;
    ;
    { //For block_3: void
      let mut var__iter_2: i64 = todo!();
      loop { //For loop_4
        ...
        { //block_7: void
          ;
          var_sum = external::op_add_int((external::op_mul_int((var_sum), (10_i32))), ({let db = (var_v); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (8_i32) as u32)} }));
        } /*block_7: void*/;
      } /*For loop_4*/;
    } /*For block_3: void*/;
    ;
    ...
    ;
    { //For block_8: void
      let mut var_r__index: i32 = todo!();
      loop { //For loop_9
        let mut var_r: DbRef = todo!();
        if !((var_r).rec != 0) { //break_10: void
          break;
        } /*break_10: void*/ else {()};
        { //block_11: void
          ;
          var_total = external::op_add_int((var_total), ({let db = (var_r); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (8_i32) as u32)} }));
        } /*block_11: void*/;
      } /*For loop_9*/;
    } /*For block_8: void*/;
    ;
    if (var_total) == (21_i32) {()} else {panic!("{}", ({ //Formatted string_12: text["__work_1"]
        var___work_1 = "Incorrect total ".to_string();
        external::format_long(&mut (var___work_1), (external::op_conv_long_from_int((var_total))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
        var___work_1
      } /*Formatted string_12: text["__work_1"]*/));};
    ;
    if !((stores.find(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, &[Content::Long((12_i32) as i64), Content::Str(Str::new(("".to_string()).as_str()))])).rec != 0) {()} else {panic!("{}", ("No element".to_string()));};
    ;
    if !((stores.find(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, &[Content::Long((83_i32) as i64), Content::Str(Str::new(("One".to_string()).as_str()))])).rec != 0) {()} else {panic!("{}", ("No element".to_string()));};
    ;
    drop((var_db));
    var_sum
  } /*block_2: integer*/;
  ;
  if (var_test_value) == (345_i32) {()} else {panic!("{}", ({ //Formatted string_13: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_long(&mut (var___work_2), (external::op_conv_long_from_int((var_test_value))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
      (var___work_2).push_str(&(" != 345".to_string()));
      var___work_2
