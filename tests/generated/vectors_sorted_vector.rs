...
fn init(db: &mut Stores) {
    ...
    let sorted_map = db.sorted(18, &[("key".to_string(), false)]);
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    ...
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    let mut var__elm_2: DbRef = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    ...
    stores.record_finish(&(var_db), &(var__elm_2), (19_i32) as u16, (0_i32) as u16);
    var__elm_2 = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    ...
    stores.record_finish(&(var_db), &(var__elm_2), (19_i32) as u16, (0_i32) as u16);
    if ({let db = (stores.find_str(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, ("Two".to_string()).as_str())); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }) == (2_i32) {()} else {panic!("{}", ("Two element".to_string()));};
    if (stores.find_str(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, ("Four".to_string()).as_str())).rec != 0 {()} else {panic!("{}", ("Four element".to_string()));};
    if !((stores.find_str(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, ("Five".to_string()).as_str())).rec != 0) {()} else {panic!("{}", ("No element".to_string()));};
    if !((stores.find_str(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, (String::new()).as_str())).rec != 0) {()} else {panic!("{}", ("Working?".to_string()));};
    let mut var_sum: i32 = 0_i32;
    { //For block_3: void
      let mut var_v__index: i32 = todo!();
      loop { //For loop_4
        let mut var_v: DbRef = todo!();
        if !((var_v).rec != 0) { //break_5: void
          break;
        } /*break_5: void*/ else {()};
        { //block_6: void
          var_sum = external::op_add_int((external::op_mul_int((var_sum), (10_i32))), ({let db = (var_v); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }));
        } /*block_6: void*/;
      } /*For loop_4*/;
    } /*For block_3: void*/;
    var_sum = external::op_add_int((external::op_mul_int((var_sum), (10_i32))), ({let db = (stores.find_str(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, ("Three".to_string()).as_str())); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }));
    drop((var_db));
    var_sum
  } /*block_2: integer*/;
  if (var_test_value) == (23143_i32) {()} else {panic!("{}", ({ //Formatted string_7: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 23143".to_string()));
      var___work_1
