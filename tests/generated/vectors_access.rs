...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    ...
    {let db = (var__elm_1); let _val = (10_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var_v), &(var__elm_1), (18_i32) as u16, (65535_i32) as u16);
    var__elm_1 = stores.record_new(&(var_v), (18_i32) as u16, (65535_i32) as u16);
    {let db = (var__elm_1); let _val = (100_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var_v), &(var__elm_1), (18_i32) as u16, (65535_i32) as u16);
    {let db = (vector::get_vector(&(var_v), (4_i32) as u32, (1_i32), &stores.allocations)); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (0_i32) as u32)} }
  } /*block_2: integer*/;
  ;
  if (var_test_value) == (10_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 10".to_string()));
      var___work_1
