...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    ...
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    var__elm_1 = stores.record_new(&(var_db), (19_i32) as u16, (0_i32) as u16);
    {let db = (var__elm_1); let _val = (61_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    {let db = (var__elm_1); let s_val = ("Seven".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = (7_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (8_i32) as u32, _val);};
    stores.record_finish(&(var_db), &(var__elm_1), (19_i32) as u16, (0_i32) as u16);
    ;
    let mut var_sum: i32 = 0_i32;
    ;
    if ({let db = (stores.find(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, &[Content::Long((83_i32) as i64), Content::Str(Str::new(("Five".to_string()).as_str()))])); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (8_i32) as u32)} }) == (5_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
        var___work_1 = "Incorrect element ".to_string();
        external::format_long(&mut (var___work_1), (external::op_conv_long_from_int(({let db = (stores.find(&(DbRef {store_nr: (var_db).store_nr, rec: (var_db).rec, pos: (var_db).pos + (0_i32) as u32}), (20_i32) as u16, &[Content::Long((83_i32) as i64), Content::Str(Str::new(("Five".to_string()).as_str()))])); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (8_i32) as u32)} }))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
        var___work_1
  ...
  ;
  if (var_test_value) == (5436_i32) {()} else {panic!("{}", ({ //Formatted string_9: text["__work_2"]
      ...
      (var___work_2).push_str(&(" != 5436".to_string()));
      var___work_2
