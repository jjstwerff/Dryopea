...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    let mut var_t: String = "some".to_string();
    (var_t).push_str(&("thing".to_string()));
    let _pre0 = t_4text_len(stores, var_t.clone());
    let _pre1 = t_4text_len(stores, var_t.clone());
    let _ret = external::op_add_int((_pre0), (_pre0));
    {};
    _ret
  } /*block_2: integer*/;
  if (var_test_value) == (18_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 18".to_string()));
      var___work_1
