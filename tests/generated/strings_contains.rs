...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: bool = { //block_2: boolean
    let mut var_t: String = "longer".to_string();
    let mut var_v: String = "a longer text".to_string();
    let _pre0 = { //Formatted string_3: text["__work_1"]
      var___work_1 = "a ".to_string();
      external::format_text(&mut (var___work_1), &(var_t), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/;
    let _ret = (var_v).contains((_pre0).as_str());
    {};
    {};
    _ret
  } /*block_2: boolean*/;
  if (var_test_value) == (true) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(if (var_test_value) {"true"} else {"false"}), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != true".to_string()));
      var___work_2
