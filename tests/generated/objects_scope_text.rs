...
fn n_test(stores: &mut Stores) { //block_1: void
  let mut var___work_1: String = "".to_string();
  ;
  let mut var_d: DbRef = stores.null();
  let mut var_test_value = String::new();
    var_test_value = { //block_2: text["d"]
    (var_d) = stores.alloc_record((18_i32) as u16);
    {let db = (var_d); let s_val = ("testing".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
    {let db = (var_d); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
    {let db = (var_d); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (0_i32) as u32) as u32))}.to_string()
  } /*block_2: text["d"]*/;
  ;
  if (var_test_value) == ("testing".to_string()) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_text(&mut (var___work_1), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&(" != \"testing\"".to_string()));
      var___work_1
