...
fn n_last(stores: &mut Stores, mut var_filename: String) -> String { //block_1: text["filename"]
  ;
  let mut var_v: i32 = if let Some(v) = (var_filename).rfind(("/".to_string()).as_str()) { v as i32 } else { i32::MIN };
  ;
  if external::op_conv_bool_from_int((var_v)) { //block_2: text["filename"]
    ;
    external::sub_text(&(var_filename), (external::op_add_int((var_v), (1_i32))), (2147483647_i32)).to_string().to_string()
  } /*block_2: text["filename"]*/ else { //block_3: text["filename"]
    ;
    var_filename
  } /*block_3: text["filename"]*/.to_string()
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    { //Add text_3: text["__work_1"]
      (var___work_1).clear();;
      (var___work_1).push_str(&(n_last(stores, "a/b/c".to_string().to_string())));
      (var___work_1).push_str(&(n_last(stores, "d".to_string().to_string())));
      var___work_1
    } /*Add text_3: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  ;
  if (var_test_value) == ("cd".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"cd\"".to_string()));
      var___work_2
