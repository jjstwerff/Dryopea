...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    ;
    let mut var_t: DbRef = stores.null();
    (var_t) = stores.alloc_record((21_i32) as u16);
    {let db = (var_t); let s_val = ("123".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var_t); let _val = i32::from((2_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    ;
    let mut var_i: DbRef = stores.null();
    (var_i) = stores.alloc_record((20_i32) as u16);
    {let db = (var_i); let _val = (101_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
    {let db = (var_i); let _val = i32::from((1_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    ;
    let mut var_a: DbRef = stores.null();
    (var_a) = stores.alloc_record((22_i32) as u16);
    {let db = (var_a); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
    let mut var__elm_1: DbRef = stores.record_new(&(var_a), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_1); let _val = (1_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var_a), &(var__elm_1), (22_i32) as u16, (1_i32) as u16);
    var__elm_1 = stores.record_new(&(var_a), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_1); let _val = (2_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var_a), &(var__elm_1), (22_i32) as u16, (1_i32) as u16);
    var__elm_1 = stores.record_new(&(var_a), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_1); let _val = (3_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var_a), &(var__elm_1), (22_i32) as u16, (1_i32) as u16);
    var__elm_1 = stores.record_new(&(var_a), (22_i32) as u16, (1_i32) as u16);
    {let db = (var__elm_1); let _val = (4_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var_a), &(var__elm_1), (22_i32) as u16, (1_i32) as u16);
    {let db = (var_a); let _val = i32::from((3_u8)); stores.store_mut(&db).set_byte(db.rec, db.pos + (0_i32) as u32, 0, _val);};
    ;
    let _pre1 = t_4Text_add(stores, var_t);
    let _pre2 = t_7Integer_add(stores, var_i);
    let _pre0 = external::op_add_int((_pre1), (_pre2));
    let _pre3 = t_5Array_add(stores, var_a);
    let _ret = external::op_add_int((_pre0), (_pre3));
    drop((var_t));
    drop((var_i));
    drop((var_a));
    _ret
  ...
  ;
  if (var_test_value) == (234_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 234".to_string()));
      var___work_1
