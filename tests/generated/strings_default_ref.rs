...
fn n_add(stores: &mut Stores, mut var_a: String, mut var_b: DbRef) -> String {
  todo!()
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    var_test_value = { //block_2: text
    let _pre0 = { //default ref_3: ref(reference)["__work_1"]
      (var___work_1).push_str(&("var".to_string()));
      todo!()
    } /*default ref_3: ref(reference)["__work_1"]*/;
    n_add(stores, "1234".to_string().to_string(), _pre0).to_string()
  } /*block_2: text*/;
  ;
  if (var_test_value) == ("var_1234".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"var_1234\"".to_string()));
      var___work_2
