...
fn n_obj(stores: &mut Stores) -> bool { //block_1: boolean
  let mut var_o: DbRef = stores.null();
  (var_o) = stores.alloc_record((18_i32) as u16);
  {let db = (var_o); let _val = (1234_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
  {let db = (var_o); let s_val = ("data".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
  {let db = (var_o); let _val = (if false {1_i32} else {0_i32}); stores.store_mut(&db).set_byte(db.rec, db.pos + (8_i32) as u32, i32::from((0_i32)), _val);};
  {let db = (var_o); let _val = (if true {1_i32} else {0_i32}); stores.store_mut(&db).set_byte(db.rec, db.pos + (8_i32) as u32, i32::from((0_i32)), _val);};
  let _ret = if if (external::op_min_int(({let db = (var_o); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (0_i32) as u32)} }), (12_i32))) == (1222_i32) {(t_4text_len(stores, {let db = (var_o); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (4_i32) as u32) as u32))}.to_string())) == (4_i32)} else {false} {({let db = (var_o); stores.store(&db).get_byte(db.rec, db.pos + (8_i32) as u32, i32::from((0_i32)))}) == (1_i32)} else {false};
  drop((var_o));
  _ret
} /*block_1: boolean*/
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    if n_obj(stores) { //block_3: integer
      1_i32
    } /*block_3: integer*/ else { //block_4: integer
      0_i32
    } /*block_4: integer*/
  } /*block_2: integer*/;
  if (var_test_value) == (1_i32) {()} else {panic!("{}", ({ //Formatted string_5: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 1".to_string()));
      var___work_1
