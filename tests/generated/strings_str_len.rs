...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    let mut var_a: String = "12345".to_string();
    let _pre1 = external::op_mul_int((t_4text_len(stores, var_a.clone())), (100_i32));
    let _pre2 = external::op_mul_int((t_4text_len(stores, "😃".to_string().to_string())), (10_i32));
    let _pre0 = external::op_add_int((_pre1), (_pre2));
    let _pre3 = t_4text_len(stores, "♥".to_string().to_string());
    let _ret = external::op_add_int((_pre0), (_pre3));
    {};
    _ret
  } /*block_2: integer*/;
  if (var_test_value) == (543_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 543".to_string()));
      var___work_1
