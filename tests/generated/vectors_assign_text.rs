...
fn init(db: &mut Stores) {
    ...
    db.field(s, "d", vec_d);
    let hash_h = db.hash(18, &["v".to_string()]);
    db.field(s, "h", hash_h);
    db.vector(18);
    db.finish();
}
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    (var_s) = stores.alloc_record((20_i32) as u16);
    {let db = (var_s); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    let mut var__elm_1: DbRef = stores.record_new(&(var_s), (20_i32) as u16, (0_i32) as u16);
    {let db = (var__elm_1); let s_val = ("a".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
    {let db = (var__elm_1); let _val = (12_i32); stores.store_mut(&db).set_short(db.rec, db.pos + (4_i32) as u32, i32::from((0_i32)), _val);};
    stores.record_finish(&(var_s), &(var__elm_1), (20_i32) as u16, (0_i32) as u16);
    {let db = (var_s); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
    ;
    {let db = (stores.get_ref(&(vector::get_vector(&(DbRef {store_nr: (var_s).store_nr, rec: (var_s).rec, pos: (var_s).pos + (0_i32) as u32}), (6_i32) as u32, (0_i32), &stores.allocations)), (0_i32) as u32)); let s_val = ("bb".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
    ;
    { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(stores.get_ref(&(vector::get_vector(&(DbRef {store_nr: (var_s).store_nr, rec: (var_s).rec, pos: (var_s).pos + (0_i32) as u32}), (6_i32) as u32, (0_i32), &stores.allocations)), (0_i32) as u32)), (18_i32) as u16));
      (var___work_1).push_str(&(" v=".to_string()));
      external::format_long(&mut (var___work_1), (external::op_conv_long_from_int(({let db = (stores.get_ref(&(vector::get_vector(&(DbRef {store_nr: (var_s).store_nr, rec: (var_s).rec, pos: (var_s).pos + (0_i32) as u32}), (6_i32) as u32, (0_i32), &stores.allocations)), (0_i32) as u32)); stores.store(&db).get_short(db.rec, db.pos + (4_i32) as u32, i32::from((0_i32)))}))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
      drop((var_s));
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  ;
  if (var_test_value) == ("{n:\"bb\",v:12} v=12".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"{n:\"bb\",v:12} v=12\"".to_string()));
      var___work_2
