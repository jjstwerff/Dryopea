  ...
  { //For block_2: void
    ...
    loop { //For loop_3
      let mut var_i: i32 = { //iter next_4: integer
        ...
        {let db = (vector::get_vector(&(var__vector_1), (4_i32) as u32, (var_i__index), &stores.allocations)); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (0_i32) as u32)} }
      } /*iter next_4: integer*/;
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    let _pre0 = { //Vector_3: vector<integer>
  ...
  ;
  if (var_test_value) == (15_i32) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 15".to_string()));
      var___work_1
    } /*Formatted string_4: text["__work_1"]*/));};
  {};
  drop((var___ref_1));
