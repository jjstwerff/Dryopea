...
fn n_test(stores: &mut Stores) { //block_1: void
  let mut var___work_1: String = "".to_string();
  let mut var_test_value: char = { //block_2: character
    let mut var_s: String = "1😊2".to_string();
    let _ret = external::text_character(&(var_s), (1_i32));
    {};
    _ret
  } /*block_2: character*/;
  if (if (var_test_value) == char::from(0) { i32::MIN } else { (var_test_value) as i32 }) == (128522_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      if let Some(c) = char::from_u32((var_test_value) as u32) { (var___work_1).push(c); };
      (var___work_1).push_str(&(" != 128522".to_string()));
      var___work_1
