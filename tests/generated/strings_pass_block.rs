...
fn n_test(stores: &mut Stores) { //block_1: void
  let mut var___work_1: String = "".to_string();
  let mut var_d: String = "".to_string();
  let mut var_test_value = String::new();
    var_test_value = { //block_2: text["d"]
    let mut var_s: String = "12 ".to_string();
    var_d = var_s;
    let _ret = (var_d).trim().to_string().to_string();
    {};
    _ret
  } /*block_2: text["d"]*/;
  if (var_test_value) == ("12".to_string()) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
