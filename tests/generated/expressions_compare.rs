...
fn n_count(stores: &mut Stores, mut var_v: u8) -> i32 { //block_1: integer
  ;
  if (if (2_u8) == 255 { i32::MIN } else { i32::from((2_u8)) }) < (if (var_v) == 255 { i32::MIN } else { i32::from((var_v)) }) { //block_2: integer
    2_i32
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    let _pre1 = n_count(stores, 1_u8);
    let _pre2 = n_count(stores, 3_u8);
    let _pre0 = external::op_add_int((_pre1), (_pre2));
    let _pre3 = n_count(stores, 3_u8);
    external::op_add_int((_pre0), (_pre2))
  } /*block_2: integer*/;
  ;
  if (var_test_value) == (5_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      ...
      (var___work_1).push_str(&(" != 5".to_string()));
      var___work_1
