...
fn n_fill(stores: &mut Stores, mut var_result: DbRef) -> DbRef { //block_1: vector<text>["result"]
  ...
  vector::clear_vector(&(var_result), &mut stores.allocations);;
  ;
  let mut var__elm_1: DbRef = stores.record_new(&(var_result), (7_i32) as u16, (65535_i32) as u16);
  {let db = (var__elm_1); let s_val = ("aa".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
  stores.record_finish(&(var_result), &(var__elm_1), (7_i32) as u16, (65535_i32) as u16);
  ;
  let mut var__elm_2: DbRef = stores.record_new(&(var_result), (7_i32) as u16, (65535_i32) as u16);
  {let db = (var__elm_2); let s_val = ("bb".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
  stores.record_finish(&(var_result), &(var__elm_2), (7_i32) as u16, (65535_i32) as u16);
  ;
  var_result
} /*block_1: vector<text>["result"]*/
