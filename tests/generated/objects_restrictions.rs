...
fn t_4Data_calc(stores: &mut Stores, mut var_self: DbRef) -> i32 { //block_1: integer
  ;
  external::op_add_int((external::op_add_int((external::op_mul_int(({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (1_i32) as u32, i32::from((1_i32)))}), (65536_i32))), (external::op_mul_int(({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (0_i32) as u32, i32::from((0_i32)))}), (256_i32))))), ({let db = (var_self); stores.store(&db).get_byte(db.rec, db.pos + (2_i32) as u32, i32::from((-127_i32)))}))
} /*block_1: integer*/
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  ;
  if (var_test_value) == (1_i32) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
