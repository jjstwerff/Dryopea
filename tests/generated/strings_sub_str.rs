...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    external::sub_text(&(var_a), (1_i32), (external::op_min_int((t_4text_len(stores, var_a.clone())), (1_i32)))).to_string().to_string()
  } /*block_2: text["a"]*/;
  if (var_test_value) == ("234".to_string()) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
