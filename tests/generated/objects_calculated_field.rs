...
fn init(db: &mut Stores) {
    ...
    db.vector(14);
    db.vector(0);
    let s = db.structure("Object", 0); // 19
    db.field(s, "name_length", 0);
    db.field(s, "name", 5);
    db.field(s, "buffer_size", 0);
    let vec_buffer = db.vector(0);
    db.field(s, "buffer", vec_buffer);
    db.finish();
}
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    (var_o) = stores.alloc_record((19_i32) as u16);
    {let db = (var_o); let s_val = ("This is example data".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (4_i32) as u32, s_pos as i32);};
    {let db = (var_o); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (12_i32) as u32, _val);};
    let mut var__elm_1: DbRef = stores.record_new(&(var_o), (19_i32) as u16, (3_i32) as u16);
    {let db = (var__elm_1); let _val = (1_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var_o), &(var__elm_1), (19_i32) as u16, (3_i32) as u16);
    var__elm_1 = stores.record_new(&(var_o), (19_i32) as u16, (3_i32) as u16);
    ...
    stores.record_finish(&(var_o), &(var__elm_1), (19_i32) as u16, (3_i32) as u16);
    var__elm_1 = stores.record_new(&(var_o), (19_i32) as u16, (3_i32) as u16);
    {let db = (var__elm_1); let _val = (5_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    stores.record_finish(&(var_o), &(var__elm_1), (19_i32) as u16, (3_i32) as u16);
    let _pre0 = t_4text_len(stores, {let db = (var_o); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (4_i32) as u32) as u32))}.to_string());
    {let db = (var_o); let _val = (_pre0); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
    let _pre1 = external::op_mul_int((t_6vector_len(stores, DbRef {store_nr: (var_o).store_nr, rec: (var_o).rec, pos: (var_o).pos + (12_i32) as u32})), (4_i32));
    {let db = (var_o); let _val = (_pre1); stores.store_mut(&db).set_int(db.rec, db.pos + (8_i32) as u32, _val);};
    { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(var_o), (19_i32) as u16));
      drop((var_o));
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  if (var_test_value) == ("{\"name_length\":20,\"name\":\"This is example data\",\"buffer_size\":20,\"buffer\":[1,2,3,4,5]}".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"{\"name_length\":20,\"name\":\"This is example data\",\"buffer_size\":20,\"buffer\":[1,2,3,4,5]}\"".to_string()));
      var___work_2
