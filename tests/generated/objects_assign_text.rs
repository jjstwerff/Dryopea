...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    var_test_value = { //block_2: text["__work_2"]
    let mut var_o: DbRef = stores.null();
    (var_o) = stores.alloc_record((18_i32) as u16);
    {let db = (var_o); let s_val = ("a".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
    ;
    {let db = (var_o); let s_val = ("b".to_string()).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
    ;
    let mut var__field_1: String = {let db = (var_o); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (0_i32) as u32) as u32))}.to_string();
    (var__field_1).push_str(&("c".to_string()));
    {let db = (var_o); let s_val = (var__field_1).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
    ;
    let mut var__field_2: String = {let db = (var_o); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (0_i32) as u32) as u32))}.to_string();
    (var__field_2).push_str(&("d".to_string()));
    (var__field_2).push_str(&("e".to_string()));
    {let db = (var_o); let s_val = (var__field_2).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
    ;
    let _pre0 = { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      external::format_text(&mut (var___work_1), &({let db = (var_o); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (0_i32) as u32) as u32))}), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&("f".to_string()));
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/;
    {let db = (var_o); let s_val = (_pre0).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
    ;
    { //Formatted string_4: text["__work_2"]
      var___work_2 = "".to_string();
      (var___work_2).push_str(&stores.rec(&(var_o), (18_i32) as u16));
      drop((var_o));
      {};
      {};
      var___work_2
    } /*Formatted string_4: text["__work_2"]*/.to_string()
  } /*block_2: text["__work_2"]*/;
  ;
  if (var_test_value) == ("{a:\"bcdef\"}".to_string()) {()} else {panic!("{}", ({ //Formatted string_5: text["__work_3"]
      var___work_3 = "Test failed ".to_string();
      external::format_text(&mut (var___work_3), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_3).push_str(&(" != \"{a:\"bcdef\"}\"".to_string()));
      var___work_3
