...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      let _pre1 = n_file(stores, "example".to_string().to_string(), var___ref_1);
      let _pre0 = t_4File_files(stores, _pre1, var___ref_2);
      (var___work_1).push_str(&stores.rec(&(_pre0), (17_i32) as u16));
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  if (var_test_value) == ("[{path:\"example/config\",size:4096,format:Directory},{path:\"example/map.png\",size:3406,format:TextFile},{path:\"example/map.xcf\",size:7817,format:TextFile},{path:\"example/show.lav\",size:371,format:TextFile},{path:\"example/todo.json\",size:1461,format:TextFile}]".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"[{path:\"example/config\",size:4096,format:Directory},{path:\"example/map.png\",size:3406,format:TextFile},{path:\"example/map.xcf\",size:7817,format:TextFile},{path:\"example/show.lav\",size:371,format:TextFile},{path:\"example/todo.json\",size:1461,format:TextFile}]\"".to_string()));
      var___work_2
