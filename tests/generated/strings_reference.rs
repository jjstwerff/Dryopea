...
fn n_add(stores: &mut Stores, mut var_a: DbRef, mut var_b: String) {
  todo!()
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    var_test_value = { //block_2: text["v"]
    var_v = "Hello".to_string();
    n_add(stores, todo!(), " world!".to_string().to_string());
    var_v
  } /*block_2: text["v"]*/;
  ;
  if (var_test_value) == ("Hello world!".to_string()) {()} else {panic!("{}", ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "Test failed ".to_string();
      external::format_text(&mut (var___work_1), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&(" != \"Hello world!\"".to_string()));
      var___work_1
