...
fn n_data(stores: &mut Stores, mut var_n: String, mut var_res: DbRef) -> DbRef { //block_1: ref(Data)["res"]
  (var_res) = stores.alloc_record((18_i32) as u16);
  {let db = (var_res); let s_val = (var_n).to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + (0_i32) as u32, s_pos as i32);};
  {let db = (var_res); let _val = (0_i32); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
  var_res
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    let mut var_d: DbRef = n_data(stores, "test".to_string().to_string(), var___ref_1);
    { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      external::format_text(&mut (var___work_1), &({let db = (var_d); let store = stores.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + (0_i32) as u32) as u32))}), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_1).push_str(&(":".to_string()));
      external::format_long(&mut (var___work_1), (external::op_conv_long_from_int(({let db = (var_d); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + (4_i32) as u32)} }))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  if (var_test_value) == ("test:0".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"test:0\"".to_string()));
      var___work_2
