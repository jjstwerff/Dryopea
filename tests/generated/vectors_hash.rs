...
fn init(db: &mut Stores) {
    ...
    db.field(s, "vector", vec_vector);
    db.vector(14);
    let s = db.structure("Keyword", 0); // 18
    db.field(s, "name", 5);
    let s = db.structure("Data", 0); // 19
    let hash_h = db.hash(18, &["name".to_string()]);
    db.field(s, "h", hash_h);
    db.finish();
}
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value: i32 = { //block_2: integer
    ...
    stores.record_finish(&(var_c), &(var__elm_2), (19_i32) as u16, (0_i32) as u16);
    ;
    if !((stores.find_str(&(DbRef {store_nr: (var_c).store_nr, rec: (var_c).rec, pos: (var_c).pos + (0_i32) as u32}), (20_i32) as u16, ("None".to_string()).as_str())).rec != 0) {()} else {panic!("{}", ("No element".to_string()));};
    ;
    if ({ //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(var_c), (19_i32) as u16));
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/) == ("{h:[{name:\"four\"},{name:\"one\"},{name:\"three\"},{name:\"two\"}]}".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
        var___work_2 = "Output hash was ".to_string();
        (var___work_2).push_str(&stores.rec(&(var_c), (19_i32) as u16));
        var___work_2
      } /*Formatted string_4: text["__work_2"]*/));};
    ;
    let _ret = if (stores.find_str(&(DbRef {store_nr: (var_c).store_nr, rec: (var_c).rec, pos: (var_c).pos + (0_i32) as u32}), (20_i32) as u16, ("three".to_string()).as_str())).rec != 0 { //block_5: integer
      12_i32
  ...
  ;
  if (var_test_value) == (12_i32) {()} else {panic!("{}", ({ //Formatted string_7: text["__work_3"]
      var___work_3 = "Test failed ".to_string();
      external::format_long(&mut (var___work_3), (external::op_conv_long_from_int((var_test_value))), (10_i32) as u8, (0_i32), (32_i32) as u8, (false), (false));
      (var___work_3).push_str(&(" != 12".to_string()));
      var___work_3
    } /*Formatted string_7: text["__work_3"]*/));};
  {};
  {};
  {};
} /*block_1: void*/
