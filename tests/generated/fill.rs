...
fn goto_false(s: &mut State) {
    ...
    if !v_if_false {s.code_pos = (s.code_pos as i32 + i32::from(v_step)) as u32;}
}
...
fn format_long(s: &mut State) {
    let v_pos = *s.code::<u16>();
    let v_radix = *s.code::<u8>();
    let v_token = *s.code::<u8>();
    let v_plus = *s.code::<bool>();
    let v_note = *s.code::<bool>();
    let v_width = *s.get_stack::<i32>();
    let v_val = *s.get_stack::<i64>();
    external::format_long(&mut v_pos, v_val, v_radix as u8, v_width, v_token as u8, v_plus, v_note)
}
...
fn format_single(s: &mut State) {
    let v_pos = *s.code::<u16>();
    let v_precision = *s.get_stack::<i32>();
    let v_width = *s.get_stack::<i32>();
    let v_val = *s.get_stack::<f32>();
    external::format_single(&mut v_pos, v_val, v_width, v_precision)
}
...
fn format_float(s: &mut State) {
    ...
    external::format_float(&mut v_pos, v_val, v_width, v_precision)
}
...
fn length_character(s: &mut State) {
    ...
    let new_value = (v_v1.len_utf8() as i32);
    s.put_stack(new_value);
}
...
fn append_text(s: &mut State) {
    let v_pos = *s.code::<u16>();
    let v_v1 = s.string();
    v_pos.push_str(&v_v1.str())
}

fn get_text_sub(s: &mut State) {
    let v_till = *s.get_stack::<i32>();
    let v_from = *s.get_stack::<i32>();
    let v_v1 = s.string();
    let new_value = external::sub_text(&v_v1.str(), v_from, v_till).to_string();
    s.put_stack(new_value);
}
...
fn conv_bool_from_character(s: &mut State) {
    let v_v1 = *s.get_stack::<char>();
    let new_value = external::op_conv_bool_from_character(v_v1);
    s.put_stack(new_value);
}
...
fn format_text(s: &mut State) {
    ...
    let v_width = *s.get_stack::<i32>();
    let v_val = s.string();
    external::format_text(&mut v_pos, &v_val.str(), v_width, v_dir as i8, v_token as u8)
}
...
fn append_character(s: &mut State) {
    let v_pos = *s.code::<u16>();
    let v_v1 = *s.get_stack::<char>();
    if let Some(c) = char::from_u32(v_v1 as u32) { v_pos.push(c); }
}
...
fn conv_int_from_character(s: &mut State) {
    let v_v1 = *s.get_stack::<char>();
    let new_value = if v_v1 == char::from(0) { i32::MIN } else { v_v1 as i32 };
    s.put_stack(new_value);
}
...
fn cast_text_from_enum(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<u8>();
    let new_value = s.database.enum_val(v_enum_tp as u16, v_v1).to_string();
    s.put_stack(new_value);
}
...
fn cast_enum_from_text(s: &mut State) {
    let v_enum_tp = *s.code::<u16>();
    let v_v1 = s.string();
    let new_value = s.database.to_enum(v_enum_tp as u16, &v_v1.str());
    s.put_stack(new_value);
}

fn conv_int_from_enum(s: &mut State) {
    let v_v1 = *s.get_stack::<u8>();
    let new_value = if v_v1 == 255 { i32::MIN } else { i32::from(v_v1) };
    s.put_stack(new_value);
}
...
fn database(s: &mut State) {
    ...
    v_pos = s.database.alloc_record(v_db_tp as u16)
}

fn format_database(s: &mut State) {
    let v_pos = *s.code::<u16>();
    let v_db_tp = *s.code::<u16>();
    let v_db_format = *s.code::<u8>();
    let v_val = *s.get_stack::<DbRef>();
    v_pos.push_str(&s.database.rec(&v_val, v_db_tp as u16))
}
...
fn var_ref(s: &mut State) {
    let v_pos = *s.code::<u16>();
    let new_value = {let r = *s.get_var::<DbRef>(v_pos); s.database.valid(&r); r};
    s.put_stack(new_value);
}
...
fn set_ref(s: &mut State) {
    ...
    let v_val = *s.get_stack::<DbRef>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = v_val.rec as i32; s.database.store_mut(&db).set_int(db.rec, db.pos + v_fld as u32, _val);}
}

fn get_field(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = DbRef {store_nr: v_v1.store_nr, rec: v_v1.rec, pos: v_v1.pos + v_fld as u32};
    s.put_stack(new_value);
}
...
fn get_int(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; if db.rec == 0 { i32::MIN } else { s.database.store(&db).get_int(db.rec, db.pos + v_fld as u32)} };
    s.put_stack(new_value);
}
...
fn get_character(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; if db.rec == 0 { char::from(0) } else { char::from_u32(s.database.store(&db).get_int(db.rec, db.pos + v_fld as u32) as u32).unwrap_or(char::from(0)) } };
    s.put_stack(new_value);
}
...
fn get_long(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_long(db.rec, db.pos + v_fld as u32)};
    s.put_stack(new_value);
}
...
fn get_single(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_single(db.rec, db.pos + v_fld as u32)};
    s.put_stack(new_value);
}
...
fn get_float(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_float(db.rec, db.pos + v_fld as u32)};
    s.put_stack(new_value);
}
...
fn get_byte(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_byte(db.rec, db.pos + v_fld as u32, i32::from(v_min))};
    s.put_stack(new_value);
}
...
fn get_enum(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_byte(db.rec, db.pos + v_fld as u32, 0) as u8};
    s.put_stack(new_value);
}
...
fn set_enum(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = i32::from(v_val); s.database.store_mut(&db).set_byte(db.rec, db.pos + v_fld as u32, 0, _val);}
}
...
fn get_short(s: &mut State) {
    ...
    let v_min = *s.code::<i16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_short(db.rec, db.pos + v_fld as u32, i32::from(v_min))};
    s.put_stack(new_value);
}
...
fn get_text(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; let store = s.database.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + v_fld as u32) as u32))};
    s.put_stack(new_value);
}
...
fn set_int(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = v_val; s.database.store_mut(&db).set_int(db.rec, db.pos + v_fld as u32, _val);}
}
...
fn set_character(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = v_val as i32; s.database.store_mut(&db).set_int(db.rec, db.pos + v_fld as u32, _val);}
}
...
fn set_long(s: &mut State) {
    ...
    let v_val = *s.get_stack::<i64>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = v_val; s.database.store_mut(&db).set_long(db.rec, db.pos + v_fld as u32, _val);}
}
...
fn set_single(s: &mut State) {
    ...
    let v_val = *s.get_stack::<f32>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = v_val; s.database.store_mut(&db).set_single(db.rec, db.pos + v_fld as u32, _val);}
}
...
fn set_float(s: &mut State) {
    ...
    let v_val = *s.get_stack::<f64>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = v_val; s.database.store_mut(&db).set_float(db.rec, db.pos + v_fld as u32, _val);}
}
...
fn set_byte(s: &mut State) {
    ...
    let v_val = *s.get_stack::<i32>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = v_val; s.database.store_mut(&db).set_byte(db.rec, db.pos + v_fld as u32, i32::from(v_min), _val);}
}
...
fn set_short(s: &mut State) {
    ...
    let v_val = *s.get_stack::<i32>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let _val = v_val; s.database.store_mut(&db).set_short(db.rec, db.pos + v_fld as u32, i32::from(v_min), _val);}
}
...
fn set_text(s: &mut State) {
    ...
    let v_val = s.string();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let s_val = v_val.str().to_string(); let store = s.database.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + v_fld as u32, s_pos as i32);}
}
...
fn vector_ref(s: &mut State) {
    let v_index = *s.get_stack::<i32>();
    let v_r = *s.get_stack::<DbRef>();
    let new_value = s.database.get_ref(&vector::get_vector(&v_r, 4, v_index, &s.database.allocations), 0);
    s.put_stack(new_value);
}
...
fn remove_vector(s: &mut State) {
    ...
    let v_index = *s.get_stack::<i32>();
    let v_r = *s.get_stack::<DbRef>();
    let new_value = vector::remove_vector(&v_r, v_size as u32, v_index, &mut s.database.allocations);
    s.put_stack(new_value);
}
...
fn new_record(s: &mut State) {
    ...
    let new_value = s.database.record_new(&v_data, v_parent_tp as u16, v_fld as u16);
    s.put_stack(new_value);
}
...
fn finish_record(s: &mut State) {
    ...
    let v_data = *s.get_stack::<DbRef>();
    s.database.record_finish(&v_data, &v_rec, v_parent_tp as u16, v_fld as u16)
}
...
fn append_vector(s: &mut State) {
    ...
    let v_r = *s.get_stack::<DbRef>();
    s.database.vector_add(&v_r, &v_other, v_tp as u16);
}

fn get_record(s: &mut State) {
    let v_db_tp = *s.code::<u16>();
    let v_no_keys = *s.code::<u8>();
    let v_data = *s.get_stack::<DbRef>();
    let new_value = todo!();
    s.put_stack(new_value);
}
...
fn iterate(s: &mut State) {
    let v_on = *s.code::<u8>();
    let v_arg = *s.code::<u16>();
    let v_keys = *s.code::<&[Key]>();
    let v_from_key = *s.code::<u8>();
    let v_till_key = *s.code::<u8>();
    let v_data = *s.get_stack::<DbRef>();
    let new_value = todo!();
    s.put_stack(new_value);
}
...
fn get_file_text(s: &mut State) {
    let v_content = *s.get_stack::<DbRef>();
    let v_file = *s.get_stack::<DbRef>();
    todo!()
}
