...
fn n_data(stores: &mut Stores, mut var_n: String, mut var_res: DbRef) -> DbRef { //block_1: vector<ref(Data)>["res"]
  ;
  vector::clear_vector(&(var_res), &mut stores.allocations);;
  ;
  ...
  { //For block_2: void
    let mut var_ch__index: i32 = 0_i32;
    loop { //For loop_3
      let mut var_ch: char = { //for text next_4: character
        let mut var__for_result_1: char = external::text_character(&(var_n), (var_ch__index));
        var_ch__index = external::op_add_int((var_ch__index), (((var__for_result_1).len_utf8() as i32)));
        var__for_result_1
      } /*for text next_4: character*/;
      ...
      { //block_6: void
        ;
        let mut var__elm_2: DbRef = stores.record_new(&(var_res), (20_i32) as u16, (65535_i32) as u16);
        {let db = (var__elm_2); let _val = (var_ch as u32 as i32); stores.store_mut(&db).set_int(db.rec, db.pos + (0_i32) as u32, _val);};
        {let db = (var__elm_2); let _val = (var_nr); stores.store_mut(&db).set_int(db.rec, db.pos + (4_i32) as u32, _val);};
        stores.record_finish(&(var_res), &(var__elm_2), (20_i32) as u16, (65535_i32) as u16);
        ;
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = String::new();
    ...
    { //Formatted string_3: text["__work_1"]
      var___work_1 = "".to_string();
      (var___work_1).push_str(&stores.rec(&(var_d), (20_i32) as u16));
      var___work_1
    } /*Formatted string_3: text["__work_1"]*/.to_string()
  } /*block_2: text["__work_1"]*/;
  ;
  if (var_test_value) == ("[{name:'t',number:0},{name:'e',number:1},{name:'s',number:2},{name:'t',number:3}]".to_string()) {()} else {panic!("{}", ({ //Formatted string_4: text["__work_2"]
      var___work_2 = "Test failed ".to_string();
      external::format_text(&mut (var___work_2), &(var_test_value), (0_i32), (-1_i32) as i8, (32_i32) as u8);
      (var___work_2).push_str(&(" != \"[{name:'t',number:0},{name:'e',number:1},{name:'s',number:2},{name:'t',number:3}]\"".to_string()));
      var___work_2
