...
| [`emit_then_measure_corrupts/`](emit_then_measure_corrupts/README.md) | ⚠⚠ **A NEGATIVE CONTROL — it does NOT reproduce**, and that is what it is for.  Two library functions iterating one struct's `vector<Struct>` field TRUNCATE it when interleaved (8 elements → 1), then the suite SIGSEGVs in `OpLengthVector`; each function alone is correct indefinitely.  Eight ingredients were tested standalone and none triggers it, so the directory records the exclusions rather than the bug. | [loft#969](https://github.com/loft-lang/loft/issues/969) |
