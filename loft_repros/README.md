...
| [`emit_then_measure_corrupts/`](emit_then_measure_corrupts/README.md) | ⚠⚠ **A NEGATIVE CONTROL — it does NOT reproduce**, and that is what it is for.  Two library functions iterating one struct's `vector<Struct>` field TRUNCATE it when interleaved (8 elements → 1), then the suite SIGSEGVs in `OpLengthVector`; each function alone is correct indefinitely.  Eight ingredients were tested standalone and none triggers it, so the directory records the exclusions rather than the bug. | [loft#969](https://github.com/loft-lang/loft/issues/969) — ⚠ **FIXED 2026-08-18**, and both builds report `loft 2026.8.0` so only the mtime tells them apart |
| [`accessor_fetch_reads_empty/`](accessor_fetch_reads_empty/README.md) | ⚠⚠ **A record fetched from a `hash<Item[name]>` THROUGH AN ACCESSOR FUNCTION reads its vector field as EMPTY** once an unrelated struct is allocated in the caller — silent, exit 0, both backends, **no imports**.  ⚠ Indexing the hash INLINE at the use site is correct, and neither the `?` discharge nor the element type is the trigger.  ⚠ With a `mesh3d::Mesh` as the allocation the same recipe sometimes ABORTS instead (*the reference is corrupt*) — which of the two you get depends on the allocation history, not on the source. | [loft#974](https://github.com/loft-lang/loft/issues/974) |

⚠ **A reproducer may be a DIRECTORY**, and seven of them are, because
