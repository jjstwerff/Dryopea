...
## Contents
- [1. `const` Has Two Different Meanings](#1-const-has-two-different-meanings)
...
- [15. `fn <name>` Function References Only Work in `par(...)` Context](#15-fn-name-function-references-only-work-in-par-context)
- [16. `Format` Enum Mixes File Mode With Absence](#16-format-enum-mixes-file-mode-with-absence)
...
## 1. `const` Has Two Different Meanings

**Severity: Medium**

The keyword `const` means fundamentally different things depending on where it appears:

| Context | Enforcement | When |
|---|---|---|
| `fn f(v: const T)` — parameter | Compile-time: any write to `v` is a type error | Always |
| `const x = expr` — local variable | Runtime: store is locked after init | Debug builds only |

```loft
fn read(v: const Counter) -> integer { v.value }   // compile-time: static proof

const c = Counter { value: 42 };                   // runtime lock (debug only)
c#lock  // true in debug, false in release
```

A programmer reading `const c = ...` expects the same guarantee as a `const` parameter.
In practice the local form provides a weaker, build-dependent check. Neither the language
reference nor error messages distinguish the two.

**Resolution path:** Either unify (make local `const` compile-time checked too) or rename
one form (e.g., `let` for local immutability intent, `const` only for parameters).
...
All four collection types use the same `+=` and `for` syntax, but only `vector` supports
the full iteration toolkit:

| Feature | `vector` | `sorted` | `index` | `hash` |
|---|---|---|---|---|
| `#first`, `#count` in loop | ✓ | ? | ? | ? |
| `#index` in loop | ✓ | ? | ? | ? |
| `v#remove` in filtered loop | ✓ | ✗ | ✗ | ✗ |
| `rev()` reverse iteration | ✓ (via range) | ✓ (fixed #17) | ✓ (fixed #17) | ✗ |
| Slicing `[a..b]` | ✓ | ✗ | ✗ | ✗ |
| Comprehension `[for x in ...]` | ✓ | ✗ | ✗ | ✗ |
| Filtered `for x in c if cond` | ✓ | ? | ? | ? |

Whether `#first`/`#count`/`#index` work on sorted/index/hash loops is not documented.
The gap is especially pronounced for `sorted` and `index`, which maintain a meaningful
order and are therefore a natural fit for "first element" or "positional index" queries.

**Resolution path:** Document which helpers apply to each collection type; implement the
missing ones or explicitly explain why they are not supported.
...
`format_int`/`format_long` in `external.rs` detect `token == b'0'` + non-empty sign and emit the sign before the zero-padded digit string.
...
## 15. `fn <name>` Function References Only Work in `par(...)` Context
...
f = fn double   // valid expression; type: fn(const Score) -> integer

for a in items par(b=double(a), 4) { sum += b; }   // WORKS
```

The `fn <name>` expression produces a compile-time-checked function reference. It is
well-typed and resolves to a definition number, but the only built-in consumer is the
`par(...)` clause. The reference cannot be stored in a struct field, passed to a
user-defined higher-order function, or used in any other context. The type exists in the
type system but has no general runtime dispatch mechanism behind it, making `fn <name>`
feel like the foundation of a feature that was not completed.

---

## 16. `Format` Enum Mixes File Mode With Absence

**Severity: Low**

```loft
f = file("path");
if f#format == NotExists { }    // file does not exist
if f#format == TextFile  { }    // file is a text file
if f#format == Directory { }    // it's a directory
```

`NotExists` is a variant of the `Format` enum alongside `TextFile`, `LittleEndian`,
`BigEndian`, and `Directory`. Semantically "the file does not exist" is absence, not a
format. Mixing absence into an enumeration of modes requires callers to check for
`NotExists` before acting on the format — similar to a nullable type but without the
clean `!f` null-check idiom.

**Resolution path:** Separate into `f#exists: boolean` and `f#format: Format` where
`Format` contains only the positive cases.
...
~~**Severity: High**~~ **Fixed 2026-03-13** (`src/external.rs`): removed `&& v2 != 0` guard from XOR/AND/OR bitwise operators. Division, remainder, and shift operators retain their guard. Tests: `tests/scripts/01-integers.loft`.
