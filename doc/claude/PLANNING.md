...
### T0-2  Index loop-remove panics "Unknown record" (use-after-free in B-tree iterator)
**Sources:** PROBLEMS #35; found by `tests/suite/21-stress.loft`
...
### T0-3  Index key-null removal in a loop leaves 1 record (off-by-one in B-tree deletion)
**Sources:** PROBLEMS #34; found by `tests/suite/21-stress.loft`
...
### T0-4  Sorted filtered loop-remove gives wrong result for large N
**Sources:** PROBLEMS #33; found by `tests/suite/21-stress.loft`
...
**Symptom:** Running `tests/suite/16-parser.loft` panics with "Incorrect store" inside
...
| T1-1 | Non-zero exit code on error                             | 1    | Low      | LOFT.md Known Limitations  |
...
| T1-7 | Null-coalescing operator `??`                           | 1    | Low      | Prototype goal             |
| T1-8 | For-loop mutation guard for field access                | 1    | Low–Med  | INCON #24                  |
