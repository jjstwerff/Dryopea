...
| `src/database.rs` | Type schema + multi-store manager (`Stores`) |
...
## Stores — Type Schema + Multi-Store Manager (`src/database.rs`)
...
These methods are surfaced to loft code via two native functions registered in `src/text.rs`:
...
- Auto-lock for local `const`: `expression()` in `src/parser.rs` — after the initialising assignment is compiled, inserts a `n_set_store_lock` call under `#[cfg(debug_assertions)]`.
- Auto-lock for const arguments: `parse_code()` in `src/parser.rs` — inserts lock calls at the start of the function body for every argument that is both an argument and const.
