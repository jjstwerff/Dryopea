# Design — Major Algorithms in Dryopea / lavition
...
| Function | File | Role |
|---|---|---|
| `parse_file()` | `src/parser.rs` | Top-level loop; calls typedef after pass 1 |
| `parse_function()` | `src/parser.rs` | `fn` declaration → body IR |
| `parse_struct()` | `src/parser.rs` | `struct` declaration |
| `parse_enum()` | `src/parser.rs` | `enum` declaration |
| `parse_type()` | `src/parser.rs` | Identifier → `Type` enum |
| `parse_operators(precedence)` | `src/parser.rs` | Precedence-climbing binary operators |
| `parse_part()` | `src/parser.rs` | Postfix `.field` and `[index]` |
| `parse_single()` | `src/parser.rs` | Atoms: literals, vars, calls, conditionals |
| `parse_assign()` | `src/parser.rs` | Assignment and compound operators |
| `parse_string()` | `src/parser.rs` | Format-string expressions |
| `convert()` / `cast()` / `can_convert()` | `src/parser.rs` | Type coercion |
| `object_init()` | `src/parser.rs` | Struct literal initialisation |
| `replace_record_ref()` | `src/parser.rs` | Substitute `Var(0)` placeholder |

**Complexity.** O(N) where N = IR nodes. Each token is visited O(1) times across the two passes combined. Precedence climbing is O(N·levels) = O(10N). Total: ~5 600 lines, the largest single file in the project.

**Reducibility.** High. The file is large because all grammar productions, type-checking, IR construction, and scope management live in one file. Splitting into `parse_decl.rs`, `parse_expr.rs`, `parse_types.rs`, and `parse_format.rs` would each be ~1 000–1 500 lines and dramatically improve navigability. The `convert`/`can_convert` family could move to `typedef.rs`. Rough saving: same total LOC but far easier maintenance.

**Code quality.** Acceptable but stretched. The single-file approach causes long scrolling distances between related functions. Pass selection via a boolean field (`self.first_pass`) is implicit — callers must remember what pass they are in. A dedicated `ParseContext` struct separating pass-1 and pass-2 state would be cleaner.
...
**Enhancement opportunities.**
- Split into four files by concern (see above).
...
| Function | File | Role |
|---|---|---|
| `byte_code(state, data)` | `src/interpreter.rs` | Iterate functions; call `def_code` for each |
| `State::def_code(d_nr, data)` | `src/state.rs` | Compile one function's IR → bytecode |
| `State::value_code(val, stack, data)` | `src/state.rs` | Recursive IR-node → opcode emitter |
| `State::code_pos()` | `src/state.rs` | Current bytecode write position |
| `State::put_code(byte)` / `put_word(u16)` / `put_long(u32)` | `src/state.rs` | Emit raw bytes |
| `State::patch(pos, offset)` | `src/state.rs` | Back-patch a forward jump target |

**Complexity.** O(N) IR nodes → O(K) opcodes where K ≈ 2–5 per IR node. Total: `state.rs` is ~2 957 lines, the second-largest file.

**Reducibility.** High. `state.rs` conflates bytecode generation, execution state, text-position tracking, and debug metadata. Splitting into `codegen.rs` and `runtime.rs` would each be ~1 000–1 500 lines and improve locality greatly.

**Code quality.** Fair. The large size makes it hard to navigate. Some generation cases (format strings, struct initialisation) are complex enough to warrant their own functions but are inline. The `HashMap`-based debug metadata (`stack`, `vars`, `calls`, `types`) is useful for testing but adds noise to the core code path.
...
**Enhancement opportunities.**
- Split `state.rs` into `codegen.rs` and `runtime.rs`.
...
**Goal.** Execute the 248-opcode bytecode by dispatching each opcode byte through a static function-pointer array `OPERATORS[opcode]`. Each operator reads inline operands from the bytecode stream and reads/writes the operand stack.

**Functions.**

| Function | File | Role |
|---|---|---|
| `OPERATORS` array | `src/fill.rs` | 248 `fn(&mut State)` pointers |
| `op_*` functions (×248) | `src/fill.rs` | Individual operator implementations |
| `State::execute()` | `src/state.rs` | Main interpreter loop |
| `State::get_stack::<T>()` / `put_stack::<T>()` | `src/state.rs` | Typed stack pop/push |
| `State::code::<T>()` | `src/state.rs` | Read inline operand from bytecode |
...
| Function | File | Role |
|---|---|---|
| `Stores::structure(name)` | `src/database.rs` | Register a struct type |
| `Stores::field(type_nr, name, field_type)` | `src/database.rs` | Add field to struct |
| `Stores::enumerate(name)` | `src/database.rs` | Register an enum type |
| `Stores::value(enum_nr, discriminant, name)` | `src/database.rs` | Add enum variant |
| `Stores::finish()` | `src/database.rs` | Seal schema; compute sizes/alignment |
| `Stores::allocate()` | `src/database.rs` | Create a new `Store` |
| `Stores::database(size)` | `src/database.rs` | Allocate a top-level record |
| `Stores::byte(min, nullable)` / `Stores::short(...)` | `src/database.rs` | Register compact integer types |
| `Stores::read_data()` / `write_data()` | `src/database.rs` | Binary serialisation |

**Complexity.** Schema registration: O(T·F) for T types with F fields each. Runtime allocation: delegates to `Store::claim` = O(B/8). ~3 646 lines — the largest file.

**Reducibility.** High. `database.rs` covers type schema, runtime store management, collection operations (sorted tree, hash, vector, radix tree dispatch), and binary I/O. Splitting into `schema.rs` (type registration), `stores.rs` (multi-store manager), and keeping collection dispatch in the collection-specific files would each be ~800–1 200 lines and much more navigable.

**Code quality.** Fair. The sheer size makes auditing hard. The `Parts` enum with 14 variants is the natural extension point for new collection types, which is good. The binary I/O routines (`read_data`/`write_data`) are complex and not tested as independently as the schema registration code.
...
**Enhancement opportunities.**
- Split into `schema.rs`, `stores.rs`, and retain collection dispatch in collection files.
...
**Goal.** Emit two Rust source files from the compiled loft default library: `fill.rs` (248 operator implementations) and `external.rs` (standard library function stubs). These are the files actually compiled into the interpreter binary.
...
| Function | File | Role |
|---|---|---|
| `text_character(val, from)` | `src/text.rs` | Get char at byte position |
| `sub_text(val, from, till)` | `src/text.rs` | Zero-copy substring |
| `fix_from(from, s)` / `fix_till(till, from, s)` | `src/text.rs` | Boundary alignment |
| `format_text(s, val, width, dir, token)` | `src/text.rs` | Pad string to width |
| `format_int(s, val, radix, width, token, plus, note)` | `src/text.rs` | Format i32 |
| `format_long(s, val, radix, width, ...)` | `src/text.rs` | Format i64 |
| `format_float(s, val, width, precision)` | `src/text.rs` | Format f64 |
| `format_single(s, val, width, precision)` | `src/text.rs` | Format f32 |
| `op_add_int` / `op_min_int` / ... (arithmetic) | `src/text.rs` | Null-sentinel arithmetic |
...
**Goal.** Provide the `lavition` binary CLI, auto-detect the project root, load the default library in alphabetical order, parse the user file, run scope analysis and bytecode generation, and execute `main`.
...
- Support reading from stdin (`lavition -`) for piped use.
...
|---|---|---|---|---|---|---|---|
| 1 | Lexer | lexer.rs | 1 000 | O(file) | No | Good | Easy |
| 2 | Two-pass parser | parser.rs | 5 600 | O(N) | **Yes** | Fair | Medium |
...
| 7 | Bytecode generation | interpreter.rs + state.rs | 3 000 | O(N) | **Yes** | Fair | Medium |
| 8 | Operator dispatch | fill.rs | 1 891 | O(I) | No (generated) | Good | Good |
| 9 | Field layout | calc.rs | 78 | O(F²) | No | Good | Easy |
| 10 | Heap allocator | store.rs | 608 | O(B/8) claim | No | Good | Medium |
| 11 | Type schema + stores | database.rs | 3 646 | O(T·F) | **Yes** | Fair | Medium |
...
| 17 | Text formatting | text.rs | 315 | O(len) | Marginal | Good | Easy |
