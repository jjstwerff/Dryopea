...
fn int_length_vector(i: &Inter, code: &[Value], state: &mut State) -> Value {
    if let Value::Reference(db, v_nr, fld) = i.calc(&code[0], state) {
        ...
        Value::Int(external::op_length_vector(store, v_nr as u32, fld as isize))
    } else {
        Value::Null
    }
}
...
fn int_add_vector(i: &Inter, code: &[Value], state: &mut State) -> Value {
    if let (Value::Reference(db, v_nr, pos), Value::Reference(odb, o_nr, opos)) =
        (i.calc(&code[0], state), i.calc(&code[1], state))
    {
        let store = &mut state.database[db as usize];
        // let ostore = &state.database[db as usize];
        // TODO for now only the same database
        // Copy data from the other array
        let (r, p) = external::op_append_vector(
            store,
            v_nr,
            pos as isize,
            external::op_length_vector(store, v_nr, pos as isize) as u32,
        );
    }
    Value::Null
}
