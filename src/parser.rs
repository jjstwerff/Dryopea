...
impl Parser {
    ...
    fn cast(&mut self, code: &mut Value, is_type: &Type, should: &Type) -> bool {
        ...
        if matches!(is_type, Type::Text(_)) && matches!(should, Type::Enum(_, true, _)) {
            *code = self.cl(
    ...
    fn can_convert(&mut self, test_type: &Type, should: &Type) -> bool {
        if *test_type != *should && !test_type.is_unknown() {
            ...
            if let (Type::Enum(_, _, _), Type::Integer(_, _)) = (test_type, should) {
                return true;
            }
            ...
            if self.lexer.has_token("{") {
                if self.first_pass {
                    if matches!(self.data.def(d_nr).returned, Type::Enum(_, false, _)) {
                       self.data.definitions[d_nr as usize].returned = Type::Enum(d_nr, true, Vec::new());
                    }
                    self.data
                ...
                self.data.definitions[v_nr as usize].returned = self.data.def(d_nr).returned.clone();
            }
            if !self.lexer.has_token(",") {
                break;
            }
        ...
        loop {
            ...
            if let Some(field) = self.lexer.has_identifier() {
                ...
                if nr == usize::MAX {
                    ...
                    let nm = self.data.attr_name(td_nr, nr);
                    let pos = self.database.position(self.data.def(td_nr).known_type, &nm);
                    if let Type::Enum(d_tp, is_ref, _) = td {
                        let mut value = Value::Null;
                        let mut parent_tp = Type::Reference(td_nr, Vec::new());
                        self.parse_operators(&td, &mut value, &mut parent_tp, 0);
                        if is_ref
                            && self.lexer.has_token("{")
                            && let Value::Enum(enum_nr, _) = value
                        {
                            let e_nr = self
                                .data
                                .def_nr(&self.data.def(d_tp).attributes[enum_nr as usize - 1].name);
                            let tp = self.data.def(e_nr).known_type;
                            list.push(Value::Drop(Box::new(self.cl(
                                "OpNewRecord",
                                &[
                                    Value::Var(v),
                                    Value::Int(i32::from(tp)),
                            ...
                            self.parse_enum_field(&mut list, Value::Var(v), d_tp, pos, enum_nr);
                        } else {
                            list.push(self.set_field(td_nr, nr, Value::Var(v), value.clone()));
                        }
                        if !self.lexer.has_token(",") {
                            break;
                        }
