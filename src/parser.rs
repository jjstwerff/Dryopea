...
pub struct Parser {
    ...
    /// - free allocations when variables run out of scope
    first_pass: bool,
    vars: Function,
}
...
impl Parser {
    ...
    fn iterator(
        ...
        if matches!(*is_type, Type::Text(_)) {
            ...
            let res_var = self.vars.work_text(&mut self.lexer);
    ...
    fn convert(&mut self, code: &mut Value, is_type: &Type, should: &Type) -> bool {
        ...
        if let Type::RefVar(ref_tp) = is_type {
            if self.convert(code, ref_tp, should) {
                return true;
            }
        }
    ...
    fn parse_code(&mut self) -> Type {
        ...
        if let Value::Block(ls) = &mut v {
            for v in self.vars.work_texts() {
                ls.insert(0, v_set(v, Value::Text(String::new())));
            }
            for v in self.vars.work_references() {
                if !self.vars.is_argument(v) {
                    ls.insert(0, v_set(v, Value::Null));
                }
            }
        }
    ...
    fn parse_assign(&mut self, code: &mut Value) -> Type {
        ...
        for op in ["=", "+=", "-=", "*=", "%=", "/="] {
            if self.lexer.has_token(op) {
                ...
                if let Type::Rewritten(tp) = s_type { s_type = *tp; }
                ...
                if let Type::RefVar(t) = &f_type && matches!(**t, Type::Text(_)) {
                    if matches!(code, Value::Insert(_)) || matches!(s_type, Type::Rewritten(_)) {
                        // nothing
                    } else if op == "=" {
                        *code = v_set(var_nr, code.clone());
                ...
                if self.create_vector(code, &f_type, op, var_nr) {
                    return Type::Void;
                }
    ...
    fn create_vector(&mut self, code: &mut Value, f_type: &Type, op: &str, var_nr: u16) -> bool {
        if let (Value::Insert(ls), Type::Vector(tp, _)) = (code, f_type) {
            if op == "=" {
                let mut val = Value::Null;
                self.vector_db(&tp, &mut val, var_nr);
                if let Value::Insert(start) = val {
                    for (s_nr, s) in start.iter().enumerate() {
    ...
    fn towards_set(&mut self, to: &Value, val: &Value, f_type: &Type, op: &str) -> Value {
        ...
        if let Type::RefVar(tp) = f_type {
            if matches!(**tp, Type::Vector(_, _) | Type::Sorted(_, _, _)) {
                if let Value::Var(nr) = to {
                    if self.vars.uses(*nr) > 0 {
                        return val.clone();
                    }
    ...
    fn parse_operators(
        ...
        loop {
            ...
            if operator == "as" {
                ...
                current_type = Type::Boolean;
            } else if operator == "+" && matches!(current_type, Type::Vector(_, _)) {
                return self.parse_append_vector(&current_type, code, parent_tp, precedence, orig_var);
    ...
    fn parse_append_vector(&mut self, current_type: &Type, code: &mut Value, parent_tp: &mut Type, precedence: usize, orig_var: u16) -> Type {
        let mut scope = u16::MAX;
        let mut ls = Vec::new();
        let mut db_var = u16::MAX;
        let mut rec_tp = if let Type::Vector(cont, _) = current_type {
            i32::from(self.data.def(self.data.type_def_nr(cont)).known_type)
        } else {
            i32::MIN
        };
        let var_nr = if orig_var == u16::MAX {
            scope = self.vars.start_scope(self.lexer.at(), "AddVector");
            let vec_def = self
                .data
                .def_nr(&format!("main_{}", current_type.name(&self.data)));
            rec_tp = i32::from(self.data.def(vec_def).known_type);
            db_var = self.vars.work_refs(&self.data.def(vec_def).returned.clone(), &mut self.lexer);
            self.vars.depend(db_var, db_var);
            ls.push(self.cl("OpClear", &[Value::Var(db_var), Value::Int(rec_tp)]));
            ls.push(self.cl("OpAppendVector", &[Value::Var(db_var), code.clone(), Value::Int(rec_tp)]));
            db_var
        } else {
            ls.push(v_set(orig_var, code.clone()));
            orig_var
        };
        let cur = self.vars.last_scope();
        let sc = self.vars.start_scope(self.lexer.at(), "AddVector");
        if self.first_pass {
            self.vars.move_scope(cur, sc);
        }
        let mut second_code = Value::Null;
        self.parse_operators(current_type, &mut second_code, parent_tp, precedence + 1);
        ls.push(self.cl("OpAppendVector", &[Value::Var(db_var), second_code.clone(), Value::Int(rec_tp)]));
        let to = if let Type::Vector(cont, _) = current_type {
            Type::Vector(cont.clone(), vec![db_var])
        } else {
            current_type.clone()
        };
        if scope != u16::MAX {
            ls.push(Value::Var(var_nr));
            self.vars.finish_scope(scope, &to, self.lexer.at());
            *code = Value::Block(ls);
            return to;
        }
        *code = Value::Insert(ls);
        Type::Rewritten(Box::new(current_type.clone()))
    }

    fn parse_append_text(&mut self, code: &mut Value, parent_tp: &mut Type, precedence: usize, orig_var: u16) -> Type {
        let mut scope = u16::MAX;
        let mut ls = Vec::new();
        let var_nr = if orig_var == u16::MAX {
            scope = self.vars.start_scope(self.lexer.at(), "AddText");
        ...
        if scope != u16::MAX {
            ls.push(Value::Var(var_nr));
            self.vars.finish_scope(scope, &tp, self.lexer.at());
            *code = Value::Block(ls);
            return tp;
        }
        *code = Value::Insert(ls);
    ...
    */
    ...
    fn parse_vector(&mut self, var_tp: &Type, val: &mut Value, parent_tp: &Type) -> Type {
        ...
        if self.lexer.has_token("]") {
            return self.empty_vector(var_tp, val, parent_tp, new_store, vec);
        }
    ...
    fn vector_db(&mut self, assign_tp: &Type, val: &mut Value, vec: u16) -> Type {
        if self.first_pass {
            ...
            *val = Value::Insert(ls);
            Type::Vector(Box::new(assign_tp.clone()), vec![db])
        }
    }
    ...
    fn parse_vector_index(
        ...
        if let Value::Iter(var, init, next) = p {
            if let Value::Block(ls) = *next {
                ...
                *code = Value::Iter(var, init, Box::new(Value::Block(vec![op])));
    ...
    fn iter_for(&mut self, val: &mut Value, append_value: &mut u16) -> Type {
        if let Some(id) = self.lexer.has_identifier() {
            ...
            let for_scope = self.vars.start_scope(self.lexer.at(), "iter for");
            self.lexer.token("in");
            ...
            *append_value = self.create_unique("val", &Type::Unknown(0));
            let loop_scope = self.vars.start_scope(self.lexer.at(), "iter for loop");
            ...
            if !matches!(in_type, Type::Iterator(_, _)) {
                lp.push(v_if(
                    self.single_op("!", Value::Var(for_var), var_tp),
                    Value::Break(0),
                    Value::Null,
                ));
            }
            ...
            lp.push(block);
            self.vars
                .finish_scope(loop_scope, &format_type, self.lexer.at());
            let tp = Type::Iterator(Box::new(format_type), Box::new(Type::Null));
            *val = Value::Iter(for_var, Box::new(create_iter), Box::new(Value::Block(lp)));
        ...
        if let Value::Iter(var, init, next) = value {
            if let Value::Block(next_ls) = &**next {
                let count = if *var == u16::MAX {
                    ...
                    let c = self.vars.find_var(&count_name);
                    if c == u16::MAX {
                        self.create_var(&count_name, &I32)
                    } else {
                        self.vars.move_var_scope(c, u16::MAX);
                        c
                    }
                };
    ...
    fn parse_object(&mut self, td_nr: u32, code: &mut Value) -> Type {
        ...
        *code = Value::Block(list);
        let mut tp = Type::Reference(td_nr, Vec::new());
        if v != u16::MAX {
            tp = tp.depending(v);
        }
        self.vars.finish_scope(
    ...
    fn parse_for(&mut self, val: &mut Value) {
        if let Some(id) = self.lexer.has_identifier() {
            ...
            lp.push(block);
            if count != u16::MAX {
                self.vars.move_var_scope(count, for_scope);
