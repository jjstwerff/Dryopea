...
impl<'a> Parser<'a> {
    ...
    fn parse_enum(&mut self) -> bool {
        ...
        loop {
            ...
            let a_nr = self
                .data
                .add_attribute(&mut self.lexer, d_nr, &value_name, Type::Integer);
            self.data.set_attr_value(d_nr, a_nr, Value::Int(nr));
            nr += 1;
            complete_definition(&mut self.lexer, &mut self.data, v_nr);
        }
    ...
    fn parse_function(&mut self) -> bool {
        ...
        let attributes = self.data.attributes(d_nr);
    ...
    fn parse_assign(&mut self, code: &mut Value) -> Type {
        ...
        self.types.change_var_type(&to, &s_type);
        if operator == "=" {
                ...
                if let Some(tps) = self.lexer.has_identifier() {
                        ...
                        && !self.cast(code, &current_type, &tp)
                    {
                        diagnostic!(
                            self.lexer,
                            Level::Error,
                            "Unknown cast from {current_type} to {tp}"
                        );
                    }
                    return tp;
                } else {
    ...
    fn parse_vector(&mut self, val: &mut Value) -> Type {
        ...
        self.types.change_var_type(&Value::Var(vec), &in_t);
        let mut ls = Vec::new();
