    ...
    fn parse_operators(
        ...
        loop {
            ...
            if operator == "as" {
                ...
                let vec_var = self.vars.work_refs(
                    self.data.def_nr(&current_type.name(&self.data)),
                    &mut self.lexer,
                );
                let mut ls = Vec::new();
                ls.push(v_set(
                ...
                ls.push(v_set(
                    vec_var,
                    self.get_field(vec_def, 0, Value::Var(db_var)),
                ));
                // TODO problem: code already contains a scope... we had to start our scope sooner or allow to change scopes
                ls.push(self.cl("OpAppendVector", &[Value::Var(vec_var), code.clone()]));
                let mut second_code = Value::Null;
                self.parse_operators(assign_tp, &mut second_code, parent_tp, precedence + 1);
                ls.push(self.cl(
                    "OpAppendVector",
                    &[Value::Var(vec_var), second_code.clone()],
                ));
                ls.push(Value::Var(vec_var));
                *code = Value::Block(ls);
