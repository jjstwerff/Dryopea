...
pub fn find(
    ...
    while rec > 0 {
        ...
        let action = if cmd == Ordering::Equal {
            if before {
                Ordering::Less
...
fn remove_elm(
    ...
    if pv.rec > 0 && right(store, &pv) < 0 {
        set_right(store, &pv, -(pos.rec as i32));
    }
    if nx.rec > 0 && left(store, &nx) < 0 {
        set_left(store, &nx, -(pos.rec as i32));
    }
    pos.rec
}
...
fn verify_walk(start: &DbRef, end: &DbRef, dir: bool, size: u32, stores: &[Store], keys: &[Key]) {
    ...
    while elm.rec != end.rec {
        ...
        assert_ne!(n.rec, 0, "Incorrect at {elm:?}");
        let mut cmp = keys::compare(&compare(&n), &compare(&elm), stores, keys);
        if dir {
            cmp = cmp.reverse();
        }
...
fn moving(store: &Store, rec: &DbRef, first: u32, second: u32) -> DbRef {
    if rec.rec == 0 {
        return to_ref(rec, 0);
    }
...
#[must_use]
pub fn next(store: &Store, rec: &DbRef) -> DbRef {
    moving(store, rec, RB_RIGHT, RB_LEFT)
}

/// Step to the previous element in the tree
#[must_use]
pub fn previous(store: &Store, rec: &DbRef) -> DbRef {
