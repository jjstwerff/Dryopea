...
impl Function {
    ...
    pub fn finish_next(&mut self, from: u16) {
        if self.logging {
            println!(
        ...
        assert_eq!(from, self.current_scope, "Problem in scopes");
        assert_eq!(from, self.stack.pop().unwrap(), "Problem in scopes");
