...
| `enemy_small_robots` | ⚠ **DELTAS, not entities** — the four roles (scout / harvester / builder / miner) carrying only what differs from `enemy_regular`, which stays the baseline rather than becoming one of the four.  Four full sections would repeat eleven fields each to vary two, and the owner (2026-08-15) expects more roles.  ⚠ `damage_to_wall` is BUILT (plan 23 K0); `speed` is written down and **not built** (K2) |
...
| `tower.repair_time_helper_rebuild` | `src/tower.loft` (plan 17 T1) | 20 s of a vehicle standing within one hex of a BLACK tower refills its magazine.  ⚠ ONE rate for the player and a helper alike, because the file has one row.  ⚠ 20.0 s is exactly 30 ticks — the epsilon class — but the timer counts UP and 20.0 counting up is exact, so `TOWER_REPAIR_EPSILON` protects the RETUNING rather than today's value (10.0 / 40.0 / 60.0 all fall short) |
...
| `enemy_regular.damage_to_wall` | `src/spawn.loft` | What a besieging enemy spends per second — and since plan 23 K0 the BASELINE of a five-row lookup (`enemy_damage_to_wall`) rather than a bare constant.  ⚠ It is untouched at 1.0, which is what let four classes land without moving one of the 569 measurements |
| `enemy_small_robots.*.damage_to_wall` | `src/spawn.loft` | The spread that makes composition legible: four of a class into one sealed band breach at **23 / 35 / 50 / 96 / 454** ticks (`@M011`).  ⚠ The scout's is priced against a real base's LENGTH (321 ticks, `@M005`) and not against the wall's nominal HP — its first value, 0.2, breached at 231 because the sum was done against the 100 HP braced figure while the siege chews the 30 HP end (`@M012`).  ⚠ **Two of those five ticks moved in K2b and no rate did**: a breach clock counts from placement, so it contains the walk, and the miner and the scout are exactly the two classes whose SPEED moved |
| `enemy_small_robots.{scout,miner}.speed` | `src/spawn.loft` | The second axis, and the one that took a phase rather than a row (plan 23 K2b).  Nine hexes of corridor in **6 / 9 / 14** ticks — scout 2.5, robot 1.5, miner 1.0 hex/s (`@M016`).  ⚠ **Harvester and builder list no speed on purpose**: this section is DELTAS, so a role that omits a field takes the regular's, and giving all four a row would invent two tunables.  ⚠⚠ **2.25 and 3.0 hex/s were refused for a rounding reason**, not a design one — both make `speed × tick_seconds` exact and hide `ENEMY_PROGRESS_EPSILON` the way 1.5 does (`@M013`), so the roster would have had no class able to see its own rounding |
| `enemy_small_robots.harvester.loot_multiplier` | `src/wallet.loft`, `src/damage.loft` | ⚠ **The hauler's whole role, through two lookups that already composed** — its body is `RUBBLE_CARGO` and `loot_rate` prices a source, so it cost one branch each and no new mechanism (`@X053`) |
