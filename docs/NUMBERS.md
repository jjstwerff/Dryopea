...
| `enemy_small_robots.*.damage_to_wall` | `src/spawn.loft` | The spread that makes composition legible: four of a class into one sealed band breach at **20 / 35 / 50 / 96 / 456** ticks (`@M011`).  ⚠ The scout's is priced against a real base's LENGTH (321 ticks, `@M005`) and not against the wall's nominal HP — its first value, 0.2, breached at 231 because the sum was done against the 100 HP braced figure while the siege chews the 30 HP end (`@M012`) |
