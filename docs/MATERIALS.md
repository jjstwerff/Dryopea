...
| **Moat** | a trench allowed to fill with water; ⚠ **many machines tolerate water badly** | ✅ **BUILT 2026-08-27** — BACKLOG C5, [`src/moat.loft`](../src/moat.loft), `@X282`.  ⚠⚠ *A trench plus the drop the palette already has* was exactly right, and *"allowed to fill"* turned out to be the load-bearing half: **a pile is a surface only once it clears the water**, so water's 1 m swallows two bodies.  ⚠ It READ *worth the whole run and it earns nothing* (`@M058`) — 378 ticks still standing against a wall's 174, on the opening 200 points, because a wave that cannot reach you cannot die.  ⚠ *Many machines tolerate water badly* is UNBUILT and is not needed: nothing on legs can enter water at all.  ⚠⚠ **BESIEGERS FILL ONE SINCE 2026-08-28** — BACKLOG C9, `@X283`: a moat is a **TIMER**, 130 / 174 / **221** (`@M059`), and *a trench allowed to fill* is now what the wave does rather than what the palette says.  ⚠⚠ **Its real job turned out not to be the barrier**: a besieger has to stand at a fixed distance and dig, so a trench in front of a tower is **335 ticks and nine of thirteen dead** (`@M060`) — a kill zone with a floor under it, at a range the player chose |
