...
| **Moat** | a trench allowed to fill with water; ⚠ **many machines tolerate water badly** | ⚠⚠ **the cheapest of the three and the one with a mechanism already waiting** — `GROUND_TYPES.md` carries water's DROP and nothing reads it, and `plans/25` § M2 names that as plan 02's decision.  A moat is *a trench plus the drop the palette already has* |
