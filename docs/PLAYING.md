...
make play SCRIPT=a-defended-base  # any of the 50 `.keys` scenarios
...
| `Shift` | `boost` | 6 hexes/sec for 2 s, then 5 s of cooldown — and it climbs **3.0 m** where a rolling vehicle climbs 0.4.  ⚠ **The only way out of a base you have sealed** — with walls, and since BACKLOG C10 with TRENCHES too: a `water` hex sits one metre below its bank and a boost is 3.0 m.  ⚠ Your crew have no boost |
...
| `F` | `dig` | ⚠⚠ **trench on / off — the other trail, and it lays a MOAT.**  Same affordance as `Q`: every hex you drive over is ordered, drive over one again and the outline is rubbed out, your crew dig it at **10 s a hex** — exactly a wall's price.  ⚠⚠ **A moat is a CUT, not a barrier you may walk along**: nothing on legs can stand in water, so it stops robots AND insects where a low wall stops only robots, and there is nothing there to chew.  ⚠⚠ **You FALL IN** — your vehicle hovers, so a trench takes you the moment you drive over it, and climbing the metre back out wants `Shift`.  Leave a gap, or carry a boost.  ⚠ Your CREW have no boost: a helper that drives into a trench is in it for the run, and the ones who dug it are standing in it.  ⚠⚠ **And it is a TIMER rather than a wall nothing can break**: a wave stopped at a trench shovels it shut, at a full wall hex's worth of chewing per metre, so a five-hex trench holds **221 ticks** against a five-hex wall's 174 (`@M059`).  ⚠⚠ **What a trench is really for is the KILL ZONE** — a besieger has to stand at a fixed distance and dig, so a tower behind one is **335 ticks and nine of thirteen dead** (`@M060`).  ⚠ You cannot dig the spoil back out; the crew clear bodies, never spoil.  ⚠ Turning it on turns `Q` off, and the reverse |
| `G` | `direct` | ⚠⚠ **tell the crew member you are standing beside what to do.**  Only works while you are **within one hex of somebody**; pressing it in an empty field does nothing.  Each press moves them one step round a cycle — **anything → build → clear → repair → re-arm → anything** — and it sticks for the rest of the run.  ⚠⚠ **A directed crew member trades BREADTH for REACH**: they will only do the one job, and they will go anywhere on the map to find it, where an undirected one takes whatever is within about three hexes of where they are standing.  ⚠ Keep pressing to come back round to *anything* — widening costs exactly what narrowing cost, which is the drive out to them.  ⚠ You never have to touch this: a crew nobody directs works
...
| ~~**a crew you can direct**~~ | ⚠⚠ **BUILT** ([`plans/29`](../plans/29-the-crews-own-work/README.md), 2026-08-28) — the crew find their own work, and **G** beside one narrows it.  ⚠ What is still missing is sending somebody to a HEX: the remit says *what*, never *where*.  ⚠ A **map** still cannot carry crew, so every shipped map is played solo and a `.keys` scenario is where a crew comes from |
