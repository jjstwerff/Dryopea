...
| **Minimal Assets** — living quarters, loneliness | [`MATERIALS.md`](MATERIALS.md) § The crew are people | ⚠ a **building a long mission needs**, once `PROGRESSION.md` § P2d gives the crew endurance pools.  A morale *bar* is still the failure mode; a pool spent by work and restored by rest is not |
...
| skills + statistics | 18 | ⚠⚠ **adopted whole, and the lattice is the artefact** — 12 skills, 6 statistics, 24 edges: every skill bonused by exactly two statistics, every statistic bonusing exactly four skills, **diameter 2**.  ⚠ The arrow runs both ways (`@X124`): practice raises the statistic, which permeates to six other skills.  ⚠ Two findings fell out of reading it as a graph — `combat` and `scout` are **twins** (both observe+stamina, the only collapsed pair), and **four pairs are free** for a future skill.  See [`PROGRESSION.md`](PROGRESSION.md) § P2a |
