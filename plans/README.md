...
| [`17-tower-hot-swap`](17-tower-hot-swap/README.md) | G | M | **Active** (T0 done) | The upkeep loop, so a base can RECOVER between waves — a crew member rebuilds a black tower in the lull, and a tower-top becomes a carry object that can be transplanted onto a spent one.  ⚠ Opened because [plan 16](16-the-wave-system/README.md) W4 named it twice: the seven-wave list plays FOUR because a tower is 300 HP of ammunition for the whole run, and a retrieval is worth ONE tick because the crew member comes back to no job.  ⚠⚠ **T0 tried to falsify the premise and could not**: without repair, building more towers buys NOTHING (3/5/7 towers -> 321/319/317 ticks, four of seven waves each), and with it seven towers CLEAR the authored list at 332.  ⚠ Its other finding is that the magazine is keyed by HEX and a hot-swap makes it a property of the TOP — so detach-and-remount must not refill |
