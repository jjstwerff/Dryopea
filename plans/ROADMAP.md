...
| Combat resolves (damage, death, rubble, towers) | **done** | [plan 12](12-combat-resolution/README.md) | Walls have structural HP and break into climbable rubble, enemies have HP and die leaving bodies that ramp a kill zone shut, towers fire at 15 hex with a sight line and a 30-shot budget, and a wallet the nibblers drain is the only end state.  ⚠ **B7's clock half-falsified the goal:** a sealed wall takes an unattended base from 161 ticks to 311, a wall with a GATE in it buys nothing at all, and adding a tower cuts it back to 180 — its own dead ramp over the wall it defends.  The missing piece is a crew to collect bodies, which arrives with the vehicle |
