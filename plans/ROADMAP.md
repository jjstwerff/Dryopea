...
| **Crew remarks on landing** | drafted | [PROGRESSION § P2c](../docs/PROGRESSION.md) | ⚠⚠ **The crew comment on the world in their own domain** (`@X129`) — an ex-miner sees ore, an ex-security officer sees approach lanes, an ex-handyman sees what the pollen is eating, an ex-scout sees distance.  ⚠ Each is right about their half and blind to the rest, so the synthesis stays the player's; a remark **points, never concludes**.  ⚠⚠ **BLOCKED on text** (`@X130`) — the first designed feature that needs `draw_text`, which `@X097` says dryopea cannot do; prerequisite is a font in the repo plus `draw_text` under `loft test` |
