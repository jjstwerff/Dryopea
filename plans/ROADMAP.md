...
| **Team matches — tower hacking and base takeover** | ⚠ **deferred behind a trigger** | [DESIGN § 20](../docs/DESIGN.md) | ⚠⚠ **The ONLY multiplayer feature needing netcode** (`@X182`) — everything else in this tier ships on persistence alone.  ⚠ **Trigger: a shipped game whose players ask for COMPETITIVE play specifically** (`@X183`), and it may never fire because the asynchronous features are already non-cooperative.  ⚠⚠ Deferring it **protects the campaign design** — building netcode early would bias bases toward matches and waves toward teams. | ⚠⚠ **2v2 / 3v3 only** (`@X179`) — hacking a tower is the one thing that would break `@X175`, and team scoping makes it safe through **consent plus a counter**: both sides signed up, and a teammate can drive to a hacked tower.  ⚠⚠ A takeover is a contest of **PRESENCE** — hacking and repairing are the same verb reversed — so the noncombatant player survives.  ⚠ **The design's first SYNCHRONOUS feature** (`@X180`): real netcode, a match lifecycle, a win condition.  ⚠⚠ **Must not risk campaign bases or crew** (`@X181`) |
