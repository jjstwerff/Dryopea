...
| NPC helpers (the crew becomes a cooperative) | **in progress** | [plan 14](14-helpers/README.md) | H0-H1 done: a crew on the player's chassis at 2.5 hex/s — ⚠ the first mover whose speed does not fit the tick, so it BANKS fractional hexes.  That is not "the tick becomes a timestep" and plan 11 F8's budget trigger does not fire.  Left: H2 (a roster that clears), H3 (wreck → retrieve → recover) |
